...
function _json5() {
    const data = /*#__PURE__*/ _interop_require_default(require("json5"));
    _json5 = function() {
        return data;
    };
    return data;
...
const _JsonFileError = /*#__PURE__*/ _interop_require_wildcard(require("./JsonFileError"));
const _writeAtomic = require("./writeAtomic");
...
function read(file, options) {
    let json;
    try {
        json = _nodefs().default.readFileSync(file, 'utf8');
    } catch (error) {
        assertEmptyJsonString(json, file);
        const defaultValue = cantReadFileDefault(options);
        if (defaultValue === undefined) {
            throw new _JsonFileError.default(`Can't read JSON file: ${file}`, error, error.code, file);
        } else {
            return defaultValue;
        }
    }
...
async function readAsync(file, options) {
    let json;
    try {
        json = await _nodefs().default.promises.readFile(file, 'utf8');
    } catch (error) {
        assertEmptyJsonString(json, file);
        const defaultValue = cantReadFileDefault(options);
        if (defaultValue === undefined) {
            throw new _JsonFileError.default(`Can't read JSON file: ${file}`, error, error.code);
        } else {
            return defaultValue;
        }
    }
...
function parseJsonString(json, options, fileName) {
    ...
    try {
        if (_getOption(options, 'json5')) {
            return _json5().default.parse(json);
        } else {
        ...
        if (defaultValue === undefined) {
            ...
            if (location) {
                ...
                const codeFrame = codeFrameColumns(json, {
                    start: location
                });
                e.codeFrame = codeFrame;
                e.message += `\n${codeFrame}`;
            }
            throw new _JsonFileError.default(`Error parsing JSON: ${json}`, e, 'EJSONPARSE', fileName);
        } else {
            return defaultValue;
        }
    }
...
function getSync(file, key, defaultValue, options) {
    ...
    if (defaultValue === undefined) {
        throw new _JsonFileError.default(`No value at key path "${String(key)}" in JSON object from: ${file}`);
    }
    return defaultValue;
}
...
function write(file, object, options) {
    if (options?.ensureDir) {
        _nodefs().default.mkdirSync(_nodepath().default.dirname(file), {
            recursive: true
        });
    }
    ...
    try {
        if (json5) {
            json = _json5().default.stringify(object, null, space);
        } else {
            json = JSON.stringify(object, null, space);
        }
    } catch (e) {
        throw new _JsonFileError.default(`Couldn't JSON.stringify object for file: ${file}`, e);
    }
    const data = addNewLineAtEOF ? `${json}\n` : json;
    (0, _writeAtomic.writeFileAtomicSync)(file, data, {
        mode: options?.mode
    });
    return object;
}
async function writeAsync(file, object, options) {
    ...
    const data = addNewLineAtEOF ? `${json}\n` : json;
    await (0, _writeAtomic.writeFileAtomic)(file, data, {
        mode: options?.mode
    });
    return object;
}
...
async function deleteKeyAsync(file, key, options) {
    return deleteKeysAsync(file, [
        key
    ], options);
}
function deleteKey(file, key, options) {
    return deleteKeys(file, [
        key
    ], options);
}
...
function assertEmptyJsonString(json, file) {
    if (json?.trim() === '') {
        throw new _JsonFileError.EmptyJsonFileError(file);
    }
}
