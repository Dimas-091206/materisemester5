...
const _ImmutableRequest = require("../../ImmutableRequest");
const _rendering = require("../../rendering");
const _matchers = require("../../utils/matchers");
...
function createEnvironment(input) {
    ...
    async function getServerRenderer() {
        ...
        ssrRenderer = async (request, options)=>{
            ...
            if (!(0, _rendering.isStreamingRenderer)(ssrModule)) {
    ...
    async function executeLoader(request, route, params) {
        ...
        return loaderModule.loader(new _ImmutableRequest.ImmutableRequest(request), params);
    }
    return {
        getRoutesManifest,
        async getHtml (request, route) {
            ...
            if (renderer) {
                ...
                const params = (0, _matchers.parseParams)(request, route);
                try {
                    if (ssrModule && (0, _rendering.isStreamingRenderer)(ssrModule) && ssrModule.resolveMetadata) {
                        renderOptions.metadata = await ssrModule.resolveMetadata({
                            route: {
                                file: route.file,
                                page: route.page
                            },
                            request: new _ImmutableRequest.ImmutableRequest(request),
                            params
                        });
                    }
                    if (route.loader) {
                        const result = await executeLoader(request, route, params);
                        const data = (0, _matchers.isResponse)(result) ? await result.json() : result;
                        renderOptions = {
                            ...renderOptions,
                            loader: {
                                data: data ?? null,
                                key: (0, _matchers.resolveLoaderContextKey)(route.page, params)
                            }
                        };
                    }
        ...
        async getLoaderData (request, route) {
            const params = (0, _matchers.parseParams)(request, route);
            const result = await executeLoader(request, route, params);
            if ((0, _matchers.isResponse)(result)) {
                return result;
            }
