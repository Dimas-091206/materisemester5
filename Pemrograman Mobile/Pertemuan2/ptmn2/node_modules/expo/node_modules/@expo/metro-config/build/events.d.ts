...
declare module '2g' {
    interface EventRegistry {
        ...
        'config:rewrite_url:redirected': {
            url: string;
        };
        'config:rewrite_url:hermes_enabled': Record<string, never>;
        'config:cache:skipped': Record<string, never>;
        'config:cache:rename_failed': {
            error: SerializedError;
        };
        'config:cache:tombstone_remove_failed': {
            tombstone: string;
            error: SerializedError;
        };
    }
}
export declare const event: import("2g").EventLogger<"config">;
