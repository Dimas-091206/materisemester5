...
Object.defineProperty(exports, "createFixHermesV1AsyncArrowNonSimpleParamsPlugin", {
    enumerable: true,
    get: function() {
        return createFixHermesV1AsyncArrowNonSimpleParamsPlugin;
    }
});
function createFixHermesV1AsyncArrowNonSimpleParamsPlugin(nox) {
    return nox.defineNativePlugin({
        ...
        visitors: [
            nox.defineVisitor('ArrowFunctionExpression', {
                ...
                if (params.every((parameter)=>{
                    if (parameter.node.type !== 'FormalParameter') return false;
                    return parameter.getChild('pattern')?.node.type === 'Identifier' && !parameter.getChild('default');
                })) return;
                ...
                if (params.some((parameter)=>parameter.node.type === 'FormalParameterRest' || parameter.node.type === 'BindingRestElement')) {
                ...
                for(let index = 0; index < params.length; index++){
                    ...
                    if (initializer) {
                        initializerParts.push('var ', pattern.getSource(), ` = ${temporary} === undefined ? `, initializer.getSource(), ` : ${temporary}; `);
                    } else {
                        initializerParts.push('var ', pattern.getSource(), ` = ${temporary}; `);
                    }
                }
                const replacementBodyParts = arrow.node.expression ? [
                    '{ ',
                    ...initializerParts,
                    'return ',
                    bodySource,
                ...
                for (const replacement of replacements){
                    ...
                    replacementParts.push(arrow.context.sourceSlice(cursor, source.start), replacement.temporary);
                    cursor = source.end;
                }
                replacementParts.push(arrow.context.sourceSlice(cursor, bodySource.start), ...replacementBodyParts);
                arrow.replaceWith({
                    kind: 'composite',
