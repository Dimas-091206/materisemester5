...
0 && (module.exports = {
    baseJSBundle: null,
    baseJSBundleWithDependencies: null,
    getBaseUrlOption: null,
    getPlatformOption: null
});
...
_export(exports, {
    ...
    get baseJSBundleWithDependencies () {
        return baseJSBundleWithDependencies;
    },
    get getBaseUrlOption () {
        return getBaseUrlOption;
    },
    get getPlatformOption () {
        return getPlatformOption;
    }
});
...
function _getAppendScripts() {
    const data = /*#__PURE__*/ _interop_require_default(require("@expo/metro/metro/lib/getAppendScripts"));
...
const _processModules = require("./processModules");
...
function baseJSBundle(entryPoint, preModules, graph, options) {
    ...
    return baseJSBundleWithDependencies(entryPoint, preModules, [
...
function baseJSBundleWithDependencies(entryPoint, preModules, dependencies, options) {
    for (const module1 of dependencies){
        options.createModuleId(module1.path);
    }
    ...
    const modules = [
        ...dependencies
    ].sort((a, b)=>options.createModuleId(a.path) - options.createModuleId(b.path));
    const sourceMapUrl = options.serializerOptions?.includeSourceMaps === false ? undefined : options.sourceMapUrl;
    const modulesWithAnnotations = (0, _getAppendScripts().default)(entryPoint, [
        ...preModules,
        ...modules
    ], {
    ...
    const mods = (0, _processModules.processModules)([
        ...dependencies
    ], processModulesOptions).map(([module1, code])=>[
            options.createModuleId(module1.path),
            code
        ]);
    return {
        ...
        paths: Object.fromEntries(mods.filter(([id, code])=>typeof code !== 'number' && Object.keys(code?.paths ?? {}).length).map(([id, code])=>[
                id,
                code.paths
            ]))
    };
