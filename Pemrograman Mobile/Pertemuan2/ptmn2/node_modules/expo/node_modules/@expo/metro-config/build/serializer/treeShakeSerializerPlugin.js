...
0 && (module.exports = {
    isModuleEmptyFor: null,
    treeShakeSerializer: null
});
...
_export(exports, {
    get isModuleEmptyFor () {
        return isModuleEmptyFor;
    },
    get treeShakeSerializer () {
        return treeShakeSerializer;
    }
});
function _core() {
...
const _reconcileTransformSerializerPlugin = require("./reconcileTransformSerializerPlugin");
...
function isModuleEmptyFor(ast) {
    ...
    (0, _core().traverse)(ast, {
        enter (path) {
            ...
            if (node.type !== 'Directive' && node.type !== 'ExpressionStatement' && !(node.type === 'Program' && node.body.length === 0)) {
...
function getExportsThatAreNotUsedInModule(ast) {
    ...
    (0, _core().traverse)(ast, {
            ...
            if (declaration) {
                if ('declarations' in declaration && declaration.declarations) {
                    declaration.declarations.forEach((decl)=>{
                        if (_core().types.isIdentifier(decl.id)) {
                            exportedIdentifiers.add(decl.id.name);
                        }
                    });
                } else if ('id' in declaration && _core().types.isIdentifier(declaration.id)) {
                    exportedIdentifiers.add(declaration.id.name);
                }
            }
            specifiers.forEach((spec)=>{
                if (_core().types.isIdentifier(spec.exported)) {
        ...
        ExportDefaultDeclaration (path) {
            ...
            if ('id' in path.node.declaration && _core().types.isIdentifier(path.node.declaration.id)) {
...
function populateModuleWithImportUsage(value) {
    for(const index in value.output){
        ...
        if (!(0, _jsOutput.isExpoJsOutput)(outputItem)) {
            continue;
        }
        ...
        (0, _assert().default)(ast, 'ast must be defined.');
        (0, _assert().default)(reconcile, 'reconcile settings are required in the module graph for post transform.');
        const deps = (0, _metrotransformworker.collectDependenciesForShaking)(ast, reconcile.collectDependenciesOptions).dependencies;
        // @ts-expect-error: Mutating the value in place.
        value.dependencies = //
        (0, _reconcileTransformSerializerPlugin.sortDependencies)(deps, value.dependencies);
    }
}
...
async function treeShakeSerializer(entryPoint, preModules, graph, options) {
    ...
    function getExportsForModule(value, checkedModules = new Set()) {
        ...
        function getDepForImportId(importModuleId) {
            ...
            const graphEntryForTargetImport = importInstance && (0, _isResolvedDependency().isResolvedDependency)(importInstance) && graph.dependencies.get(importInstance.absolutePath);
            // Should never happen but we're playing with fire here.
            if (!graphEntryForTargetImport) {
                throw new Error(`Failed to find graph key for re-export "${importModuleId}" while optimizing ${value.path}. Options: ${[
                    ...value.dependencies.values()
                ].map((v)=>v.data.name).join(', ')}`);
            }
                        ...
                        if (exportResults.isStatic && !exportResults.hasUnresolvableStarExport) {
                            ...
                            for (const exportName of exportResults.exportNames){
                                if (exportName !== 'default' && !exportNames.has(exportName)) {
                                    ...
                                    exportSpecifiers.push(_core().types.exportSpecifier(_core().types.identifier(exportName), _core().types.identifier(exportName)));
                                }
                            }
                            path.replaceWith(_core().types.exportNamedDeclaration(null, exportSpecifiers, _core().types.stringLiteral(path.node.source.value)));
                            // TODO: Update deps
    ...
    function disposeOfGraphNode(nodePath) {
        ...
        for (const dep of node.dependencies.values()){
            if (!(0, _isResolvedDependency().isResolvedDependency)(dep)) continue;
    ...
    function getDependencyHashIdForImportModuleId(graphModule, importModuleId) {
        ...
        // The hash key for the dependency instance in the module.
        const depId = [
            ...graphModule.dependencies.entries()
        ].find(([_key, dep])=>{
            return dep.data.name === importModuleId;
        })?.[0];
        // // Should never happen but we're playing with fire here.
        if (!depId) {
            throw new Error(`Failed to find graph key for import "${importModuleId}" from "${importModuleId}" while optimizing ${graphModule.path}. Options: ${[
                ...graphModule.dependencies.values()
            ].map((v)=>v.data.name).join(', ')}`);
        }
        return depId;
    }
    function disconnectGraphNode(graphModule, importModuleId, { isSideEffectyImport, bailOnDuplicateDefaultExport } = {}) {
        ...
        // Should never happen but we're playing with fire here.
        if (!graphEntryForTargetImport) {
            throw new Error(`Failed to find graph key for re-export "${importModuleId}" while optimizing ${graphModule.path}. Options: ${[
                ...graphModule.dependencies.values()
            ].map((v)=>v.data.name).join(', ')}`);
        }
        //
        ...
        if (bailOnDuplicateDefaultExport && // @ts-expect-error: exportNames is added by babel
        ...
        const [authorMarkedSideEffect] = (0, _sideEffects.hasSideEffectWithDebugTrace)(options, graph, graphEntryForTargetImport);
        ...
        // let trace: string[] = [];
        if (// Don't remove the module if it has side effects.
        !isFx || // Unless it's an empty module.
        isEmptyModule(graphEntryForTargetImport)) {
            ...
            (0, _assert().default)('imports' in importData, 'Expo InternalDependency type expected, but `imports` key is missing.');
        ...
        for(const index in value.output){
            ...
            (0, _core().traverse)(ast, {
                ...
                ExportNamedDeclaration (path) {
                    ...
                    if (_core().types.isExportNamedDeclaration(path.node)) {
                        for(let i = 0; i < path.node.specifiers.length; i++){
                            const specifier = path.node.specifiers[i];
                            if (_core().types.isExportSpecifier(specifier) && _core().types.isIdentifier(specifier.local) && _core().types.isIdentifier(specifier.exported) && possibleUnusedExports.includes(specifier.exported.name) && !isExportUsed(specifier.exported.name)) {
                                // Remove specifier
                    ...
                    if (_core().types.isVariableDeclaration(declaration)) {
                    ...
                    } else if (declaration && 'id' in declaration && _core().types.isIdentifier(declaration.id)) {
                        // function, class, etc.
                        if (possibleUnusedExports.includes(declaration.id.name) && !isExportUsed(declaration.id.name)) {
                            // TODO: Update source maps
                            markUnused(path);
    ...
    function removeUnusedImportsFromModule(value, ast) {
        ...
        (0, _core().traverse)(ast, {
            ImportSpecifier (path) {
                if (// Support `import { foo as bar } from './foo'`
                path.node.local.name != null) {
                    importedIdentifiers.add(path.node.local.name);
                } else if (// Support `import { foo } from './foo'`
                _core().types.isIdentifier(path.node.imported) && path.node.imported.name != null) {
            ...
            Identifier (path) {
                ...
                if (!path.scope.bindingIdentifierEquals(path.node.name, path.node) && // `import { Foo as Bar } from 'bax'` Foo should not be marked as used.
        ...
        const unusedImports = [
            ...importedIdentifiers
        ].filter((identifier)=>!usedIdentifiers.has(identifier));
        ...
        importDecs.forEach((path)=>{
            ...
            if (originalSize !== path.node.specifiers.length) {
                ...
                const targetHashId = getDependencyHashIdForImportModuleId(value, importModuleId);
                const importInstance = value.dependencies.get(targetHashId);
                if (importInstance && (0, _isResolvedDependency().isResolvedDependency)(importInstance)) {
    ...
    function removeUnusedImports(value) {
        ...
        const dirtyImports = value.output.map((outputItem)=>{
            // TODO: Module type should be properly upcast
    ...
    function optimizePaths(paths) {
        ...
        while(paths.length){
            ...
            markDirty(// Order is important (not sure why though)
            ...removeUnusedExports(dep), // Remove imports after exports
            ...
            dep.dependencies.forEach((dep)=>{
                if ((0, _isResolvedDependency().isResolvedDependency)(dep)) paths.push(dep.absolutePath);
            });
        }
    }
}
