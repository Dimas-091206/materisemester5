...
Object.defineProperty(exports, "createMetroLiveBindingsPlugin", {
    enumerable: true,
    get: function() {
        return createMetroLiveBindingsPlugin;
    }
});
const _noxcturnaltransformer = require("../noxcturnal-transformer");
const _metrodependency = require("./metro-dependency");
function createMetroLiveBindingsPlugin(nox, liveBindings = true) {
    const generateUid = (scope, input)=>{
        let name = input.replace(/[^$0-9A-Z_a-z]/g, '-').replace(/^[-0-9]+/, '');
        name = name.replace(/[-\s]+(.)?/g, (_match, next)=>next ? next.toUpperCase() : '');
    ...
    const exportsName = (context)=>{
        const { normalizePseudoGlobals, pseudoGlobals } = (0, _noxcturnaltransformer.metroPluginData)(context);
        return normalizePseudoGlobals ? pseudoGlobals.exports : 'exports';
    };
    const liveExport = (name, expression, target = 'exports')=>`Object.defineProperty(${target}, ${JSON.stringify(name)}, { enumerable: true, get: function () { return ${expression}; } });`;
    const exportValue = (name, expression, target = 'exports')=>liveBindings ? liveExport(name, expression, target) : `${target}.${name} = ${expression};`;
  ...
  return n;
    ...
    return nox.defineNativePlugin({
        ...
        post (context, state) {
            const { collectOnly } = (0, _noxcturnaltransformer.metroPluginData)(context);
            ...
            for (const deferred of state.deferredExports){
                if (deferred.kind === 'declaration') {
                    const statement = (!liveBindings || deferred.assign) && deferred.name !== '__proto__' ? `${target}.${deferred.name} = ${deferred.name};` : exportValue(deferred.name, deferred.name, target);
                ...
                for (const module of state.moduleOrder){
                    if (module.defaultLocal != null && (expression === module.defaultLocal || expression.startsWith(`${module.defaultLocal}.`)) || module.namespaceLocal != null && expression === module.namespaceLocal || expression.startsWith(`${module.requiredLocal}.`)) {
                        module.referenced = true;
                        if (module.defaultLocal != null && (expression === module.defaultLocal || expression.startsWith(`${module.defaultLocal}.`))) {
            ...
            const preamble = [];
            if (state.exportStatements.length > 0 || trailingExports.length > 0 || state.moduleOrder.some((module)=>module.afterImport.some((operation)=>operation.kind === 'statement'))) {
                preamble.push(`Object.defineProperty(${target}, '__esModule', { value: true });`);
            }
            const emittedInterops = new Set();
            for (const module of state.moduleOrder){
                for (const operation of module.afterImport){
                    const interop = operation.kind === 'default-interop' ? 'default' : operation.kind === 'namespace-interop' ? 'namespace' : null;
                    if (interop != null && !emittedInterops.has(interop) && (interop === 'default' ? module.defaultReferenced : module.namespaceReferenced)) {
            ...
            for (const module of state.moduleOrder){
                ...
                for (const operation of module.afterImport){
                    if (operation.kind === 'statement' || operation.kind === 'default-interop' && module.defaultReferenced || operation.kind === 'namespace-interop' && module.namespaceReferenced) {
        ...
        visitors: [
            nox.defineVisitor('ImportDeclaration', {
                fields: [
                    'source',
                    'importKind'
                ],
                ...
                const { collectOnly, shared, input } = (0, _noxcturnaltransformer.metroPluginData)(importPath.context);
                state.sawEsm = true;
                ...
                if (collectOnly) {
                    ...
                    (0, _metrodependency.registerNativeEsmDependency)(dependencyState, source, importPath.getLocation(), importPath.getChildList('specifiers').map((specifier)=>specifier.node.type === 'ImportNamespaceSpecifier' ? '*' : specifier.node.type === 'ImportDefaultSpecifier' ? 'default' : String(specifier.node.imported)));
                    return;
                }
                const specifiers = importPath.getChildList('specifiers');
                const valueSpecifiers = specifiers.filter((specifier)=>specifier.node.type !== 'ImportSpecifier' || !specifier.node.importKind || specifier.node.importKind === 'value');
                ...
                if (!module) {
                    const dependencyState = shared.state;
                    if (!dependencyState) importPath.unsupported('missing-shared-dependency-state');
                    const dependency = (0, _metrodependency.registerNativeEsmDependency)(dependencyState, source, importPath.getLocation());
                    ...
                    module = {
                        ...
                        sideEffect: input.options.dev && importPath.context.metadata.get('performConstantFolding') !== true
                    };
                    state.modules.set(source, module);
                    state.moduleOrder.push(module);
                }
                if (specifiers.length === 0) module.sideEffect = true;
                const bindings = new Map(importPath.scope.getBindings(valueSpecifiers.map((specifier)=>String(specifier.node.local))).map((binding)=>[
                        binding.name,
                        binding
                    ]));
                for (const specifier of specifiers){
                    if (specifier.node.type === 'ImportSpecifier' && specifier.node.importKind && specifier.node.importKind !== 'value') {
                        continue;
                    }
                    ...
                    if (specifier.node.type === 'ImportNamespaceSpecifier') {
                        ...
                        const imported = specifier.node.type === 'ImportDefaultSpecifier' ? 'default' : String(specifier.node.imported);
                        if (imported === 'default') {
                    ...
                    for (const reference of binding?.references ?? []){
                        ...
                        if (specifier.node.type === 'ImportNamespaceSpecifier') {
                            module.namespaceReferenced = true;
                        } else if (specifier.node.type === 'ImportDefaultSpecifier' || String(specifier.node.imported) === 'default') {
                            module.defaultReferenced = true;
                        }
                        ...
                        const preservesLocalReference = expression.startsWith(`${local}.`) || expression.startsWith(`${local}[`);
                        if (preservesLocalReference) {
                            if (reference.parentType === 'CallExpression' && reference.parentStart === reference.start) {
                                importPath.context.editor.appendLeft(reference.start, '(0, ');
                                importPath.context.editor.appendRight(reference.end, `${expression.slice(local.length)})`);
                            } else if (reference.parentType === 'ObjectProperty' && reference.parentStart === reference.start) {
                                importPath.context.editor.appendRight(reference.end, `: ${expression}`);
                            } else {
                                importPath.context.editor.appendRight(reference.end, expression.slice(local.length));
                            }
                        } else {
                            const replacement = reference.parentType === 'CallExpression' && reference.parentStart === reference.start ? `(0, ${expression})` : reference.parentType === 'ObjectProperty' && reference.parentStart === reference.start ? `${local}: ${expression}` : expression;
                            importPath.context.editor.overwrite(reference.start, reference.end, replacement, 'anchor-start');
                        }
                    }
                }
                importPath.remove();
            }),
            nox.defineVisitor('ExportNamedDeclaration', {
                fields: [
                    'source',
                    'exportKind'
                ],
                ...
                const { collectOnly, shared } = (0, _noxcturnaltransformer.metroPluginData)(exportPath.context);
                state.sawEsm = true;
                ...
                if (exportPath.node.source) {
                    ...
                    const valueExportSpecifiers = exportSpecifiers.filter((specifier)=>!specifier.node.exportKind || specifier.node.exportKind === 'value');
                    if (collectOnly) {
                        const dependencyState = shared.state;
                        if (!dependencyState) exportPath.unsupported('missing-shared-dependency-state');
                        (0, _metrodependency.registerNativeEsmDependency)(dependencyState, source, exportPath.getLocation(), exportSpecifiers.map((specifier)=>String(specifier.node.local)));
                        return;
                    }
                    ...
                    if (!module) {
                        const dependencyState = shared.state;
                        if (!dependencyState) exportPath.unsupported('missing-shared-dependency-state');
                        const dependency = (0, _metrodependency.registerNativeEsmDependency)(dependencyState, source, exportPath.getLocation());
                        const dependencyArgument = `${dependencyState.dependencyMapName}[${dependency.index}]`;
                    ...
                    for (const specifier of valueExportSpecifiers){
                        ...
                        if (imported === 'default') {
                            if (module.defaultLocal == null) {
                                module.defaultLocal = generateUid(exportPath.scope.getProgramParent(), requiredLocal);
                                module.afterImport.push({
                                    kind: 'default-interop',
                                    code: `var ${module.defaultLocal} = _interopDefault(${module.requiredLocal});`
                                });
                        ...
                        state.exportStatements.push(exportValue(String(specifier.node.exported), expression, exportsName(exportPath.context)));
                    }
                    exportPath.remove();
                    return;
                }
                    ...
                    if (typeof declaration.node.name === 'string') {
                        ...
                        declaration.traverse([
                            nox.defineVisitor('Identifier', {
                                ...
                                if (identifier.node.binding === true && identifier.scope.id === exportPath.scope.id) {
            ...
            nox.defineVisitor('ExportDefaultDeclaration', {
                fields: [
                    'declaredName'
                ],
                scope: true,
                ...
                const { collectOnly } = (0, _noxcturnaltransformer.metroPluginData)(exportPath.context);
                state.sawEsm = true;
                if (collectOnly) return;
                const declaration = exportPath.getChild('declaration');
                ...
                if (local) {
                    ...
                    if (declaration.node.type === 'Class' || declaration.node.type === 'Function') {
                        ...
                        exportPath.replaceWith(exportPath.context.code.template`var ${local} = ${declaration.getSource()};`);
                    }
                }
                if (liveBindings) state.exportStatements.push(exportValue('default', local, exportsName(exportPath.context)));
                else state.deferredExports.push({
                    kind: 'specifier',
                    local,
                    exported: 'default'
                });
            }),
            nox.defineVisitor('ExportAllDeclaration', {
                fields: [
                    'source',
                    'exportKind',
                    'exported'
                ...
                if (collectOnly) {
                    const dependencyState = shared.state;
                    if (!dependencyState) exportPath.unsupported('missing-shared-dependency-state');
                    (0, _metrodependency.registerNativeEsmDependency)(dependencyState, source, exportPath.getLocation(), [
                        '*'
                    ]);
                    return;
                }
                ...
                if (typeof exportPath.node.exported === 'string') {
                    ...
                    module.namespaceReferenced = true;
                    state.exportStatements.push(exportValue(exportPath.node.exported, module.namespaceLocal, exportsName(exportPath.context)));
                } else {
                    if (!module.exportAll) {
                        module.exportAll = true;
                        const target = exportsName(exportPath.context);
                        const code = liveBindings ? `Object.keys(${requiredLocal}).forEach(function (k) {
  ...
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(${target}, k)) {
    ${target}[k] = ${requiredLocal}[k];
                        ...
                        if (liveBindings) module.afterImport.push({
                            kind: 'statement',
                            code
                        });
...
//# sourceMappingURL=metro-live-bindings.js.map
