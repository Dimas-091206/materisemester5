...
Object.defineProperty(exports, "createReactNativeWebPlugin", {
    enumerable: true,
    get: function() {
        return createReactNativeWebPlugin;
    }
});
const REACT_NATIVE_WEB_EXPORTS = new Set([
    'AccessibilityInfo',
    'ActivityIndicator',
    'Alert',
    'Animated',
...
function reactNativeWebLocation(name) {
    const internal = name === 'unstable_createElement' ? 'createElement' : name;
    return REACT_NATIVE_WEB_EXPORTS.has(internal) ? `react-native-web/dist/exports/${internal}` : null;
}
function createReactNativeWebPlugin(nox) {
    const isRoot = (source)=>source === 'react-native' || source === 'react-native-web';
    return nox.defineNativePlugin({
        ...
        visitors: [
            nox.defineVisitor('ImportDeclaration', {
                ...
                importPath.replaceWithMultiple(specifiers.map((specifier)=>{
            ...
            nox.defineVisitor('ExportNamedDeclaration', {
                ...
                if (specifiers.length === 0) return;
                exportPath.replaceWithMultiple(specifiers.map((specifier)=>{
                    ...
                    return location ? `export { default as ${exported} } from ${JSON.stringify(location)};` : `export { ${specifier.sourceText()} } from "react-native-web/dist/index";`;
                }));
            }),
            nox.defineVisitor('VariableDeclaration', {
                fields: [
                    'kind'
                ],
                children: {
                ...
                for (const declarator of declaration.getChildList('declarations')){
                    ...
                    if (!id || init?.node.type !== 'CallExpression' || init.node.calleeName !== 'require' || init.node.calleeGlobal !== true || !isRoot(init.node.staticArgument)) {
                        retained.push(declarator);
                        continue;
                    }
                    if (id.node.type === 'Identifier') {
                        replacements.push(`${kind} ${String(id.node.name)} = require("react-native-web/dist/index");`);
                        changed = true;
                        continue;
                    }
                    ...
                    for (const property of properties){
                        if (property.node.type !== 'BindingProperty' || property.node.computed === true || typeof property.node.key !== 'string') {
                            valid = false;
                            break;
                        }
                        const value = property.getChild('value');
                        if (!value || value.node.type !== 'Identifier' || typeof value.node.name !== 'string') {
                            valid = false;
                            break;
                        }
                        supported.push({
                    ...
                    for (const property of supported){
                        ...
                        if (location) {
                            replacements.push(`${kind} ${property.local} = require(${JSON.stringify(location)}).default;`);
                        } else {
