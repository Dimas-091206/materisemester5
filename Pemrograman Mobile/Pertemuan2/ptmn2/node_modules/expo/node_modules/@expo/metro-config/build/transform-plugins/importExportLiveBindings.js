...
Object.defineProperty(exports, "importExportLiveBindingsPlugin", {
    enumerable: true,
    get: function() {
        return importExportLiveBindingsPlugin;
    }
});
...
function importExportLiveBindingsPlugin({ template, types: t }) {
    ...
    const addImport = (path, state, source, sideEffect = false)=>{
        ...
        let id = moduleSpecifiers["REQUIRE"];
        if (!id) {
            id = path.scope.generateUid(source.value);
            moduleSpecifiers["REQUIRE"] = id;
    ...
    const addDefaultImport = (path, state, source, name)=>{
        const moduleSpecifiers = addModuleSpecifiers(state, source);
        let id = moduleSpecifiers["DEFAULT"];
        if (!id) {
            ...
            id = !name || !t.isValidIdentifier(name) ? path.scope.generateUid(name ?? parentImportLocal) : name;
            moduleSpecifiers["DEFAULT"] = id;
            state.importDeclarations.push({
                kind: "DEFAULT",
    ...
    const addNamespaceImport = (path, state, source, name)=>{
        const moduleSpecifiers = addModuleSpecifiers(state, source);
        let id = moduleSpecifiers["NAMESPACE"];
        if (!id) {
            // Use the given name, if possible, or generate one. If no initial name is given,
            // we'll create one based on the parent import
            const parentImportLocal = addImport(path, state, source);
            id = !name || !t.isValidIdentifier(name) ? path.scope.generateUid(name ?? parentImportLocal) : name;
            moduleSpecifiers["NAMESPACE"] = id;
            state.importDeclarations.push({
                kind: "NAMESPACE",
                local: parentImportLocal,
                source,
        ...
        visitor: {
                ...
                for (const specifier of path.node.specifiers){
                    ...
                    switch(specifier.type){
                        ...
                        case 'ImportSpecifier':
                            ...
                            member = t.isIdentifier(specifier.imported) ? specifier.imported.name : specifier.imported.value;
                            // An imported default specifier is the same as an ImportDefaultSpecifier
                            importId = member === 'default' ? addDefaultImport(path, state, source, localId) : addImport(path, state, source);
                            break;
            ...
            ExportAllDeclaration (path, state) {
                ...
                if (!state.exportAll.has(importId)) {
                    ...
                    state.exportAll.set(importId, (0, _helpers.withLocation)((0, _helpers.liveExportAllHelper)(template, importId), loc));
                }
                path.remove();
            },
            ...
            ExportDefaultDeclaration (path, state) {
                ...
                if (t.isDeclaration(path.node.declaration)) {
                    ...
                    path.replaceWith((0, _helpers.withLocation)((0, _helpers.varDeclaratorHelper)(t, localId, path.node.declaration), path.node.loc));
                }
                state.exportDeclarations.push({
                    statement: (0, _helpers.withLocation)((0, _helpers.liveExportHelper)(t, 'default', t.identifier(localId)), path.node.loc),
                    local: undefined
                });
            },
                ...
                for (const specifier of path.node.specifiers){
                    ...
                    switch(specifier.type){
                        ...
                        case 'ExportSpecifier':
                            ...
                            // An imported default specifier is the same as an ImportDefaultSpecifier
                            importId = specifierLocal === 'default' ? addDefaultImport(path, state, source) : addImport(path, state, source);
                    ...
                    state.referencedLocals.add(importId);
                    state.exportDeclarations.push({
                        statement: (0, _helpers.withLocation)((0, _helpers.liveExportHelper)(t, exportName, exportExpression), path.node.loc),
                        local: importId
                    });
                }
                path.remove();
                ...
                exit (path, state) {
                    ...
                    for (const exportStatement of state.exportStatements){
                        ...
                        if (!exportStatement.source && exportStatement.specifiers) {
                            for (const specifier of exportStatement.specifiers){
                                ...
                                state.exportDeclarations.push({
                                    statement: (0, _helpers.withLocation)((0, _helpers.liveExportHelper)(t, exportName, exportExpression), exportStatement.loc),
                                    local: undefined
                                });
                            }
                        }
                        ...
                        if (declaration) {
                            ...
                            const exportHelper = declaration.type === 'VariableDeclaration' || declaration.type !== 'FunctionDeclaration' ? _helpers.liveExportHelper : _helpers.assignExportHelper;
                            ...
                            for(const exportName in exportBindings){
                                if (exportBindings[exportName] != null) {
                                    state.exportDeclarations.push({
                                        statement: (0, _helpers.withLocation)(exportHelper(t, exportName, t.identifier(exportBindings[exportName].name)), exportStatement.loc),
                                        local: undefined
                                    });
                                }
                            }
                    ...
                    path.traverse({
                        ReferencedIdentifier (path, state) {
                            ...
                            if (inlineRefExpression) {
                                ...
                                if (path.parent.type === 'CallExpression' && path.parent.callee === path.node && inlineRefExpression.type !== 'JSXMemberExpression') {
                                    inlineRefExpression = (0, _helpers.nullBoundExpression)(t, inlineRefExpression);
                                }
                    ...
                    const wrapDefault = (localId, sourceId)=>{
                        if (!_defaultWrapName) {
                            ...
                            preambleStatements.push((0, _helpers.defaultWrapHelper)(template, _defaultWrapName));
                        }
                        return (0, _helpers.varDeclaratorCallHelper)(t, localId, _defaultWrapName, sourceId);
                    };
                    let _namespaceWrapName;
                    const wrapNamespace = (localId, sourceId)=>{
                        if (!_namespaceWrapName) {
                            _namespaceWrapName = '_interopNamespace';
                            preambleStatements.push((0, _helpers.namespaceWrapHelper)(template, _namespaceWrapName));
                        }
                        return (0, _helpers.varDeclaratorCallHelper)(t, localId, _namespaceWrapName, sourceId);
                    };
                    // Add `__esModule` marker if we have any exports
                    if (state.exportDeclarations.length || state.exportAll.size) {
                        preambleStatements.push((0, _helpers.esModuleExportTemplate)(template));
                    }
                    ...
                    for (const importDeclaration of state.importDeclarations){
                        ...
                        if (!local || !state.referencedLocals.has(local)) {
                            ...
                            if (importDeclaration.kind === "REQUIRE" && moduleSpecifiers.sideEffect) {
                                esmStatements.push((0, _helpers.withLocation)((0, _helpers.sideEffectRequireCall)(t, source), importDeclaration.loc));
                            }
                            continue;
                        }
                        let importStatement;
                        switch(importDeclaration.kind){
                            case "REQUIRE":
                                importStatement = (0, _helpers.requireCall)(t, local, source);
                                break;
                            case "DEFAULT":
                        ...
                        importStatement = (0, _helpers.withLocation)(importStatement, importDeclaration.loc);
