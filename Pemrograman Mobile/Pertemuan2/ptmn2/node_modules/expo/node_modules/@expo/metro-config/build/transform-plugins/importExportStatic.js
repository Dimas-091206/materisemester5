...
Object.defineProperty(exports, "importExportPlugin", {
    enumerable: true,
    get: function() {
...
const _events = require("../transform-worker/events");
...
const importTemplate = _core().template.statement(`
  var %%asId%% = %%importHelperId%%(%%moduleStr%%);
`);
const requireNameTemplate = _core().template.statement(`
  var %%asId%% = require(%%moduleStr%%).%%nameId%%;
`);
const requireSideEffectTemplate = _core().template.statement(`
  require(%%moduleStr%%);
`);
...
const staticExportAllTemplate = _core().template.statements(`
...
`);
const staticExportTemplate = _core().template.statement(`
  exports.%%asId%% = %%nameId%%;
`);
// Needs to be kept in 1:1 compatibility with Babel.
const esModuleExportTemplate = _core().template.statement(`
  Object.defineProperty(exports, '__esModule', {value: true});
`);
const resolveTemplate = _core().template.expression(`
        ...
        visitor: {
            ...
            ExportDefaultDeclaration (path, state) {
                ...
                if (isDeclaration(declaration)) {
                    path.insertBefore((0, _helpers.withLocation)(declaration, loc));
                } else {
                    path.insertBefore((0, _helpers.withLocation)(t.variableDeclaration('var', [
                ...
                if (declaration) {
                    if (isVariableDeclaration(declaration)) {
                                ...
                                case 'ObjectPattern':
                                    {
                                        ...
                                        properties.forEach((p)=>{
                                            ...
                                            if (name) {
                                                ...
                                                (0, _events.debugEvent)('import_export:unexpected_object_pattern', {
                                                    node: p.toString()
                                                });
                                            }
                                        });
                                ...
                                case 'ArrayPattern':
                                    {
                                        ...
                                        elements.forEach((e)=>{
                                            ...
                                            if (name) {
                                                ...
                                                (0, _events.debugEvent)('import_export:unexpected_array_pattern', {
                                                    node: e?.toString() ?? ''
                                                });
                                            }
                                        });
                                ...
                                default:
                                    {
                                        ...
                                        if (name) {
                                            ...
                                            (0, _events.debugEvent)('import_export:unexpected_identifier', {
                                                node: id.toString()
                                            });
                                        }
                                    }
                        ...
                        if ('id' in declaration) {
                            ...
                            (0, _events.debugEvent)('import_export:unexpected_declaration', {
                                node: declaration.toString()
                            });
                        }
                    }
                    ...
                    specifiers.forEach((s)=>{
                        ...
                        switch(s.type){
                            ...
                            case 'ExportNamespaceSpecifier':
                                ...
                                break;
                            default:
                                (0, _events.debugEvent)('import_export:unexpected_specifier', {
                                    node: s.toString()
                                });
                                break;
                        }
            ...
            ImportDeclaration (path, state) {
                ...
                if (!specifiers.length) {
                    state.importSideEffect.set(file.value, {
                        loc
                    });
                } else {
                    specifiers.forEach((s)=>{
            ...
            Program: {
                enter (path, state) {
                    ...
                    [
                        ...
                        'exports',
                        'require'
                    ].forEach((name)=>path.scope.rename(name));
                },
                exit (path, state) {
                    const body = path.node.body;
                    const hasESExports = state.exportDefault.length || state.exportAllFrom.size || state.exportNamed.length || state.exportAllFromAs.size || state.exportNamedFrom.size;
                    const hasESImports = state.importAllFromAs.size || state.importDefaultFromAs.size || state.importNamedFrom.size || state.importSideEffect.size;
                    const imports = [];
                    const exportAll = [];
                    const staticExports = [];
                    ...
                    for (const [, module] of state.originalImportOrder){
                        ...
                        if (exportAllFrom) {
                            // export * from 'module'
                            exportAll.push(...(0, _helpers.withLocation)(staticExportAllTemplate({
                        ...
                        for (const { as, loc } of getArray(state.exportAllFromAs, module.value)){
                            ...
                            imports.push((0, _helpers.withLocation)(importTemplate({
                            ...
                            staticExports.push((0, _helpers.withLocation)(staticExportTemplate({
                        ...
                        for (const { name, as, loc } of getArray(state.exportNamedFrom, module.value)){
                            if (name === 'default' && as === 'default') {
                                ...
                                defaultStaticExports.push((0, _helpers.withLocation)(staticExportTemplate({
                                ...
                                const tmp = path.scope.generateUidIdentifier(name);
                                imports.push((0, _helpers.withLocation)(requireNameTemplate({
                                    moduleStr: t.cloneNode(resolved),
                        ...
                        if (getArray(state.importNamedFrom, module.value).length === 1) {
                            ...
                            for (const { name, as, loc } of getArray(state.importNamedFrom, module.value)){
                                if (!sharedModuleVariableDeclaration) {
                                    ...
                                    sharedModuleVariableDeclaration = (0, _helpers.withLocation)(t.variableDeclaration('var', [
                                        t.variableDeclarator(t.cloneNode(sharedModuleImport), t.callExpression(t.identifier('require'), [
                                            t.cloneNode(resolved)
                                        ]))
                                    ]), loc);
                                    imports.push(sharedModuleVariableDeclaration);
                                }
                                sharedModuleVariableDeclaration.declarations.push((0, _helpers.withLocation)(t.variableDeclarator(t.identifier(as), t.memberExpression(t.cloneNode(nullthrows(sharedModuleImport)), t.identifier(name))), loc));
                            }
                        }
                        if (!sharedModuleVariableDeclaration && state.importSideEffect.has(module.value)) {
                            // import 'module' -> require('module')
                            imports.push((0, _helpers.withLocation)(requireSideEffectTemplate({
                                moduleStr: t.cloneNode(resolved)
