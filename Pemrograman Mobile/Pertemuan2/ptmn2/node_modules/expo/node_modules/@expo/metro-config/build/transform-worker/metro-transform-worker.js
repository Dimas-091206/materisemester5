...
0 && (module.exports = {
    InvalidRequireCallError: null,
    applyImportSupport: null,
    collectDependenciesForShaking: null,
    getCacheKey: null,
    minifyCode: null,
    transform: null
});
...
_export(exports, {
    get InvalidRequireCallError () {
        return InvalidRequireCallError;
    },
    get applyImportSupport () {
        return applyImportSupport;
    },
    get collectDependenciesForShaking () {
        return collectDependenciesForShaking;
    },
    get getCacheKey () {
        return getCacheKey;
    },
...
function _metrocachekey() {
    const data = require("@expo/metro/metro-cache-key");
...
function _metrosourcemap() {
    const data = require("@expo/metro/metro-source-map");
...
function _metrotransformplugins() {
    const data = /*#__PURE__*/ _interop_require_wildcard(require("@expo/metro/metro-transform-plugins"));
...
function _generateImportNames() {
    const data = /*#__PURE__*/ _interop_require_default(require("@expo/metro/metro/ModuleGraph/worker/generateImportNames"));
...
const _packedMap = require("../serializer/packedMap");
const _sourceMap = require("../serializer/sourceMap");
const _transformplugins = require("../transform-plugins");
const _assettransformer = /*#__PURE__*/ _interop_require_wildcard(require("./asset-transformer"));
const _collectdependencies = /*#__PURE__*/ _interop_require_wildcard(require("./collect-dependencies"));
const _events = require("./events");
const _metrotransformworker = require("./noxcturnal/metro-transform-worker");
const _resolveOptions = require("./resolveOptions");
const _getMinifier = require("./utils/getMinifier");
...
function isDefaultExpoBabelTransformer(transformerPath, transformer) {
    const defaultTransformer = require('../babel-transformer');
    ...
    return /[/\\]@expo[/\\]metro-config[/\\](?:build|src)[/\\]babel-transformer\.[cm]?js$/.test(transformerPath);
}
function hasNonDefaultProjectBabelConfig(transformer, args) {
...
const minifyCode = async (config, filename, code, source, rawMappings, reserved = [], inputSourceMap)=>{
    const sourceMap = inputSourceMap ?? (0, _sourceMap.rawMappingsToEncodedMap)({
        filename,
        source,
        rawMappings
    });
    const minify = (0, _getMinifier.getMinifier)(config.minifierPath);
    ...
    try {
        const minified = await minify({
            ...
            file: _events.debugEvent.path(filename)
        });
        return {
            code: minified.code,
            sourceMap: minified.map ? (0, _packedMap.packDecodedMappings)({
                mappings: minified.map.mappings,
                names: minified.map.names
            }) : (0, _packedMap.emptySourceMap)()
        };
    } catch (error) {
...
function getImportNames(options, ast) {
    if (options.experimentalImportSupport === true && options.customTransformOptions?.liveBindings !== 'false') {
    ...
    return (0, _generateImportNames().default)(ast);
}
function applyImportSupport(ast, { filename, options, importDefault, importAll, collectLocations, performConstantFolding }) {
    ...
    if (collectLocations) {
        plugins.push(// TODO: Only enable this during reconciling.
    ...
    if (options.experimentalImportSupport === true) {
        ...
        plugins.push([
            liveBindings ? _transformplugins.importExportLiveBindingsPlugin : _transformplugins.importExportPlugin,
            {
                ...babelPluginOpts,
    ...
    if (options.inlineRequires) {
        plugins.push([
            _metrotransformplugins().inlineRequiresPlugin,
            {
                ...babelPluginOpts,
    ...
    if (plugins.length) {
        const done = _events.debugEvent.span();
        const result = nullthrows((0, _core().transformFromAstSync)(ast, '', {
            ast: true,
            babelrc: false,
            code: false,
            configFile: false,
...
function performConstantFolding(ast, { filename }) {
    ...
    // transform).
    const done = _events.debugEvent.span();
    const result = (0, _core().transformFromAstSync)(ast, '', {
        ast: true,
        babelrc: false,
        code: false,
        configFile: false,
        ...
        plugins: [
            ...
            _metrotransformplugins().constantFoldingPlugin
        ],
        sourceMaps: false,
...
async function transformJS(file, { config, options }) {
    ...
    const optimize = // Ensure we don't enable tree shaking for scripts or assets.
    file.type === 'js/module' && String(options.customTransformOptions?.optimize) === 'true' && // Disable tree shaking on JSON files.
    ...
    let ast = file.ast ?? nullthrows((0, _core().parse)(file.code, {
    ...
    if (!options.dev || file.performConstantFolding) {
        ast = performConstantFolding(ast, {
    ...
    if (file.type === 'js/script') {
        dependencies = [];
        wrappedAst = _JsFileWrapping().wrapPolyfill(ast);
    } else {
        try {
            ...
            collectDependenciesOptions = {
                ...
                dynamicRequires: isServerEnv ? // This requires a patch to Metro collectDeps.
                'warn' : getDynamicDepsBehavior(config.dynamicDepsInPackages, file.filename),
                inlineableCalls: [
                    importDefault,
                    importAll
                ],
            ...
            const doneDeps = _events.debugEvent.span();
            ({ ast, dependencies, dependencyMapName } = (0, _collectdependencies.default)(ast, {
                ...
                unstable_isESMImportAtSource: importDeclarationLocs != null ? (loc)=>importDeclarationLocs.has((0, _importLocationsPlugin().locToKey)(loc)) : null
            }));
        ...
        if (unstable_disableModuleWrapping === true) {
            ...
            ({ ast: wrappedAst } = _JsFileWrapping().wrapModule(ast, importDefault, importAll, dependencyMapName, config.globalPrefix, // TODO: This config is optional to allow its introduction in a minor
            // release. It should be made non-optional in ConfigT or removed in
            // future.
            unstable_renameRequire === false));
        }
    }
    const minify = (0, _resolveOptions.shouldMinify)(options);
    const shouldNormalizePseudoGlobals = minify && file.inputFileSize <= config.optimizationSizeLimit && !config.unstable_disableNormalizePseudoGlobals;
    const reserved = [];
    if (config.unstable_dependencyMapReservedName != null) {
        reserved.push(config.unstable_dependencyMapReservedName);
    }
    if (shouldNormalizePseudoGlobals && // TODO: If the module wrapping is disabled then the normalize function needs to change to account for not being in a body.
    !unstable_disableModuleWrapping) {
        ...
        reserved.push(..._metrotransformplugins().normalizePseudoGlobals(wrappedAst, {
            reservedNames: reserved
        }));
    }
    const doneCodegen = _events.debugEvent.span();
    ...
    const generatedSourceMap = file.inputSourceMap ? (0, _sourceMap.composeSourceMaps)([
        {
            version: 3,
            ...
            sourcesContent: [
                file.inputSourceMap.originalCode
            ]
        },
        (0, _sourceMap.rawMappingsToEncodedMap)({
            filename: file.filename,
            source: file.code,
            rawMappings
        })
    ...
    if (minify) {
        ({ sourceMap, code } = await minifyCode(config, file.filename, result.code, file.code, rawMappings, reserved, generatedSourceMap ? {
            ...generatedSourceMap,
            version: 3,
            sources: generatedSourceMap.sources.map((source)=>source ?? file.filename)
        } : undefined));
    } else {
        sourceMap = generatedSourceMap ? (0, _packedMap.packDecodedMappings)({
            mappings: generatedSourceMap.mappings,
            names: generatedSourceMap.names
        }) : (0, _packedMap.packRawMappings)(rawMappings);
    }
...
/** Transforms an asset file. */ async function transformAsset(file, context) {
    const { assetRegistryPath, assetPlugins } = context.config;
    // TODO: Add web asset hashing in production.
    const result = await _assettransformer.transform(getBabelTransformArgs(file, context), assetRegistryPath, assetPlugins);
    const jsFile = {
        ...file,
 ...
 */ async function transformJSWithBabel(file, context) {
    ...
    const sourceDefaultTransformer = require('../babel-transformer');
    ...
    const hasNonDefaultBabelConfig = isDefaultExpoTransformer && hasNonDefaultProjectBabelConfig(transformer.isDefaultConfig ? transformer : sourceDefaultTransformer, babelConfigArgs);
    if (!(0, _metrotransformworker.isNoxcturnalTransformWorkerEnabled)(context.config)) {
        enableExperimentalImportSupportForReactCompiler(context);
        return transformJSWithBabelFallback(file, context, transformer);
    }
    const noxcturnal = await (0, _metrotransformworker.tryTransformJSWithNoxcturnal)(file, context, {
...
function enableExperimentalImportSupportForReactCompiler(context) {
...
async function completeFullNoxcturnalTransform(file, context, fullNoxcturnal) {
    if (String(context.options.customTransformOptions?.optimize) === 'true') {
        return transformJS({
            ...
            hasCjsExports: typeof fullNoxcturnal.result.metadata.hasCjsExports === 'boolean' ? fullNoxcturnal.result.metadata.hasCjsExports : file.hasCjsExports,
            reactServerReference: typeof fullNoxcturnal.result.metadata.reactServerReference === 'string' ? fullNoxcturnal.result.metadata.reactServerReference : file.reactServerReference,
            reactClientReference: typeof fullNoxcturnal.result.metadata.reactClientReference === 'string' ? fullNoxcturnal.result.metadata.reactClientReference : file.reactClientReference,
            expoDomComponentReference: typeof fullNoxcturnal.result.metadata.expoDomComponentReference === 'string' ? fullNoxcturnal.result.metadata.expoDomComponentReference : file.expoDomComponentReference,
            loaderReference: typeof fullNoxcturnal.result.metadata.loaderReference === 'string' ? fullNoxcturnal.result.metadata.loaderReference : file.loaderReference,
            functionMap: fullNoxcturnal.result.functionMap ?? file.functionMap
        }, context);
    }
    let code = fullNoxcturnal.result.code;
    let sourceMap;
    if ((0, _resolveOptions.shouldMinify)(context.options)) {
        const reserved = context.config.unstable_dependencyMapReservedName == null ? [] : [
            context.config.unstable_dependencyMapReservedName
        ];
        ({ code, sourceMap } = await minifyCode(context.config, file.filename, code, file.code, [], reserved, {
            version: 3,
            sources: [
                file.filename
            ],
        ...
        sourceMap = (0, _packedMap.packDecodedMappings)({
            mappings: fullNoxcturnal.result.map.mappings,
            names: fullNoxcturnal.result.map.names
        });
    }
...
async function transformJSWithBabelFallback(file, context, transformer) {
    ...
    const jsFile = {
        ...
        functionMap: transformResult.metadata?.metro?.functionMap ?? // Fallback to deprecated explicitly-generated `functionMap`
        transformResult.functionMap ?? null,
...
async function transformJSON(file, { options, config }) {
    let code = config.unstable_disableModuleWrapping === true ? _JsFileWrapping().jsonToCommonJS(file.code) : _JsFileWrapping().wrapJson(file.code, config.globalPrefix);
    let sourceMap = (0, _packedMap.emptySourceMap)();
    const minify = (0, _resolveOptions.shouldMinify)(options);
    if (minify) {
...
function getCacheKey(config, opts) {
    const { // The `expo_customTransformerPath` from `./supervising-transform-worker` should not be part of the cache key
    ...
    const filesKey = (0, _metrocachekey().getCacheKey)([
        ...
        (0, _getMinifier.resolveMinifier)(minifierPath),
        ...
        ..._metrotransformplugins().getTransformPluginCacheKeyFiles(),
        ...(0, _metrotransformworker.getNoxcturnalCacheKeyFiles)()
    ]);
    ...
    const babelTransformerCacheKey = babelTransformer.getCacheKey ? babelTransformer.getCacheKey({
    ...
    keyParts.push(filesKey, (0, _metrocache().stableHash)(remainingConfig).toString('hex'), babelTransformerCacheKey);
    return keyParts.join('$');
}
/**
 * Produces a Babel template that transforms an "import(...)" call into a
 * "require(...)" call to the asyncRequire specified.
 */ const makeShimAsyncRequireTemplate = _core().template.expression(`require(ASYNC_REQUIRE_MODULE_PATH)`);
...
function collectDependenciesForShaking(ast, options) {
    ...
    return (0, _collectdependencies.default)(ast, {
        ...collectDependenciesOptions,
        // This setting shouldn't be shared with the tree shaking transformer.
