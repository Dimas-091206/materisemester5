...
Object.defineProperty(exports, "createEnvironmentRestrictedReactApisPlugin", {
    enumerable: true,
    get: function() {
        return createEnvironmentRestrictedReactApisPlugin;
    }
});
...
function createEnvironmentRestrictedReactApisPlugin(nox, skip = ()=>false) {
    const forbiddenFor = (source)=>source === 'react' ? INVALID_SERVER_REACT_APIS : source === 'react-dom' ? INVALID_SERVER_REACT_DOM_APIS : null;
    return nox.defineNativePlugin({
        ...
        visitors: [
            ...
            nox.defineVisitor('VariableDeclarator', {
                scope: true,
                children: {
                    init: {
                ...
                const api = binding?.declaration?.id == null ? undefined : state.named.get(binding.declaration.id);
                if (api) declarator.unsupported(`react-server-client-api-use:${api}`);
            }),
            nox.defineVisitor('MemberExpression', {
                scope: true,
                children: {
                    object: {
                        route: 'Identifier',
                ...
                const source = binding?.declaration?.id == null ? undefined : state.namespaces.get(binding.declaration.id);
...
//# sourceMappingURL=environment-restricted-react-apis.js.map
