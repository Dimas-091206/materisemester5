...
_export(exports, {
    get INTERNAL_CALLSITES_REGEX () {
        return INTERNAL_CALLSITES_REGEX;
    },
    get getDefaultCustomizeFrame () {
        return getDefaultCustomizeFrame;
    }
});
function _nodepath() {
...
function isUrl(value) {
    ...
    if (_nodepath().default.isAbsolute(value)) {
        return false;
    }
    try {
        // eslint-disable-next-line no-new
        new (_nodeurl()).URL(value);
        return true;
    } catch  {
        return false;
    }
...
function getDefaultCustomizeFrame() {
    return (frame)=>{
        ...
        let collapse = Boolean(frame.file && INTERNAL_CALLSITES_REGEX.test((0, _filePath.toPosixPath)(frame.file)));
        if (!collapse) {
            // This represents the first frame of the stacktrace.
            // Often this looks like: `__r(0);`.
            // The URL will also be unactionable in the app and therefore not very useful to the developer.
            if (frame.column === 3 && frame.methodName && [
                'global',
                'global code'
            ].includes(frame.methodName) && frame.file?.match(/^https?:\/\//g)) {
                collapse = true;
            } else if ((frame.file === 'unknown' || frame.file === '<anonymous>') && (frame.column == null || frame.column === -1)) {
                ...
                collapse = true;
            } else if (// Some internal component stacks often don't have a file name.
            frame.file === '<anonymous>' && frame.methodName && [
                // React
                'Suspense',
                // React Native
                'RCTView',
