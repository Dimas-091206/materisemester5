...
0 && (module.exports = {
    applyStaticHeaders: null,
    loadStaticManifestAsync: null,
    resolveStaticHeaders: null
});
...
function resolveStaticHeaders(manifest, pathname) {
    let mergedHeaders = manifest.headers ?? {};
    for (const rule of manifest.pageHeaders ?? []){
        if (rule.namedRegex.test(pathname)) {
            mergedHeaders = (0, _private().mergeHeaderInputs)(mergedHeaders, rule.headers);
        }
    }
