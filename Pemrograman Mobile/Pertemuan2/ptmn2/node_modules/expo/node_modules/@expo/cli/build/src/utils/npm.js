...
0 && (module.exports = {
    downloadAndExtractNpmModuleAsync: null,
    extractLocalNpmTarballAsync: null,
    extractNpmTarballAsync: null,
    extractNpmTarballFromUrlAsync: null,
    getNpmUrlAsync: null,
    normalizeNpmPackResult: null,
    npmViewAsync: null,
    packNpmTarballAsync: null,
    sanitizeNpmPackageName: null
});
...
async function npmViewAsync(...props) {
    ...
    const results = (await (0, _spawnasync().default)('npm', cmd)).stdout?.trim();
    const cmdString = `npm ${cmd.join(' ')}`;
    (0, _events.event)('npm_run', {
        command: cmdString
    });
    if (!results) {
        return null;
...
async function packNpmTarballAsync(packageDir) {
    ...
    const results = (await (0, _spawnasync().default)('npm', cmdArgs, {
        env: {
            ...process.env
        },
        cwd: packageDir
