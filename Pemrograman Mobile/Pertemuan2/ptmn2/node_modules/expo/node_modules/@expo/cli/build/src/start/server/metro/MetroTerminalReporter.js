...
0 && (module.exports = {
    MetroTerminalReporter: null,
    event: null,
    extractCodeFrame: null,
    formatUsingNodeStandardLibraryError: null,
    isNodeStdLibraryModule: null,
    stripMetroInfo: null
});
...
class MetroTerminalReporter extends _TerminalReporter.TerminalReporter {
   ...
   */ _getBundleStatusMessage(progress, phase) {
        ...
        const localPath = typeof progress.bundleDetails?.customTransformOptions?.dom === 'string' && progress.bundleDetails.customTransformOptions.dom.includes(_path().default.sep) ? progress.bundleDetails.customTransformOptions.dom.replace(/^(\.?\.[\\/])+/, '') : this.#normalizePath(progress.bundleDetails.entryFile);
        if (!inProgress) {
        ...
        if ((0, _interactive.shouldReduceLogs)()) {
            return '';
        }
    ...
    #captureLog(evt) {
        switch(evt.type){
            case 'bundle_build_started':
                {
                    const customTransformOptions = evt.bundleDetails?.customTransformOptions;
                    const entry = typeof customTransformOptions?.dom === 'string' && customTransformOptions.dom.includes(_path().default.sep) ? customTransformOptions.dom.replace(/^(\.?\.[\\/])+/, '') : this.#normalizePath(evt.bundleDetails.entryFile);
...
/** @returns platform specific tag for a `BundleDetails` object */ function getEnvironmentForBuildDetails(bundleDetails) {
    ...
    const customTransformOptions = bundleDetails?.customTransformOptions;
