...
0 && (module.exports = {
    WebpackBundlerDevServer: null,
    getProjectWebpackConfigFilePath: null
});
...
_export(exports, {
    get WebpackBundlerDevServer () {
        return WebpackBundlerDevServer;
    },
    get getProjectWebpackConfigFilePath () {
...
function assertIsWebpackDevServer(value) {
    if (!value?.sockWrite && !value?.sendMessage) {
        throw new _errors.CommandError('WEBPACK', value ? 'Expected Webpack dev server, found: ' + (value.constructor?.name ?? value) : 'Webpack dev server not started yet.');
    }
}
class WebpackBundlerDevServer extends _BundlerDevServer.BundlerDevServer {
    ...
    async startImplementationAsync(options) {
        ...
        (0, _nodeassert().default)(options.port, 'Expected a port to be defined before starting the Webpack dev server');
        ...
        _log.log((0, _chalk().default)`Starting Webpack on port ${this.getPort()} in {underline ${mode}} mode.`);
        // Create a webpack compiler that is configured with custom messages.
        const compiler = webpack(config);
        const server = new WebpackDevServer(compiler, config.devServer);
        const host = _env.env.WEB_HOST ?? (options.location.hostType === 'localhost' ? 'localhost' : undefined);
        // Launch WebpackDevServer.
        server.listen(this.getPort(), host, function(error) {
            if (error) {
