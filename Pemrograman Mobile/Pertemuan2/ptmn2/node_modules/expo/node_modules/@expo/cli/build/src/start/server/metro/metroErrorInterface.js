...
0 && (module.exports = {
    IS_METRO_BUNDLE_ERROR_SYMBOL: null,
    attachImportStackToRootMessage: null,
    dropStackIfContainsCodeFrame: null,
    getErrorOverlayHtmlAsync: null,
    getStackAsFormattedLog: null,
    likelyContainsCodeFrame: null,
    logMetroError: null,
    logMetroErrorAsync: null,
    logMetroErrorWithStack: null,
    nearestImportStack: null
});
...
function _logboxutils() {
    const data = require("@expo/log-box-utils");
    _logboxutils = function() {
        return data;
    };
    return data;
}
...
const isDebug = _env.env.EXPO_DEBUG;
function fill(width) {
    ...
    // from Babel and we should not duplicate it as message is already printed before this call.
        ...
        if (isPreviewTooLong) {
            ...
            if (lineText && column != null) {
                const rangeWindow = Math.round(Math.max(codeFrame.fileName?.length ?? 0, Math.max(80, process.stdout.columns)) / 2);
...
/** @returns the html required to render the static metro error as an SPA. */ function logFromError({ error, projectRoot }) {
    const data = (0, _logboxutils().parseWebBuildErrors)({
        error,
        projectRoot,
        parseErrorStack
    });
...
const attachImportStackToRootMessage = (err, importStack = nearestImportStack(err))=>{
