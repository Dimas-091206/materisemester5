...
const expoRunIos = async (argv)=>{
    ...
    if (args['--help']) {
        (0, _args.printHelp)(`Run the iOS app binary locally`, `npx expo run:ios`, [
            ...
            (0, _chalk().default)`-d, --device [device]            Device name, UDID, or "generic" for build-only`,
            ...
            '',
            (0, _chalk().default)`  Build for simulator without installing (build-only):`,
            (0, _chalk().default)`    {dim $} npx expo run:ios --configuration Release --device generic --output ./build`,
            ''
        ].join('\n'));
    }
