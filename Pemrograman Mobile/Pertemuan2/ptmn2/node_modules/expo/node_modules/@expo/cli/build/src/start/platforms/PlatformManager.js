...
const event = (0, _2g().events)('platform');
class PlatformManager {
   ...
   */ async getExpoGoOrCustomRuntimeUrlAsync(deviceManager) {
        ...
        if (redirectUrl) {
            ...
            if (applicationId) {
                (0, _events.event)('open_launch_url', {
                    appId: applicationId,
                    redirectUrl
                });
    ...
    async openProjectInCustomRuntimeAsync(resolveSettings = {}, props = {}) {
        (0, _events.event)('open_custom', {
            props: Object.entries(props).map(([k, v])=>`${k}: ${v}`).join(', ')
        });
        let url = this.props.getCustomRuntimeUrl({
            scheme: props.scheme
        });
        (0, _events.event)('open_custom_url', {
            url,
