...
0 && (module.exports = {
    WebSupportProjectPrerequisite: null,
    isWebPlatformExcluded: null
});
...
_export(exports, {
    get WebSupportProjectPrerequisite () {
        return WebSupportProjectPrerequisite;
    },
    get isWebPlatformExcluded () {
...
class WebSupportProjectPrerequisite extends _Prerequisite.ProjectPrerequisite {
    ...
    /** Exposed for testing. */ async _ensureWebDependenciesInstalledAsync({ exp, pkg }) {
        ...
        try {
            ...
            if (hasReactDOM) {
                _log.warn((0, _chalk().default)`{bold react-native-web} is not installed. Some React Native components may not work on web without it. Install it with: {bold npx expo install react-native-web}`);
                return false;
            }
            throw error;
        }
...
function isWebPlatformExcluded(rootConfig) {
    // Detect if the 'web' string is purposefully missing from the platforms array.
    const isWebExcluded = Array.isArray(rootConfig?.expo?.platforms) && !!rootConfig.expo?.platforms.length && !rootConfig.expo?.platforms.includes('web');
