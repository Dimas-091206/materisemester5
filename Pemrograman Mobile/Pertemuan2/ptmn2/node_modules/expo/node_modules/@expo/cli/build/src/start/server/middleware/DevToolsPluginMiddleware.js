...
0 && (module.exports = {
    DevToolsPluginEndpoint: null,
    DevToolsPluginMiddleware: null
});
...
const _devtoolsEvents = require("../devtoolsEvents");
...
class DevToolsPluginMiddleware extends _ExpoMiddleware.ExpoMiddleware {
    ...
    shouldHandleRequest(req) {
        if (!req.url?.startsWith(_DevToolsPluginManager.DevToolsPluginEndpoint)) {
            return false;
        }
        return true;
    }
   ...
   */ async handleWithPluginServerAsync(plugin, req, res, urlInPluginRoot) {
        ...
        try {
            ...
            (0, _devtoolsEvents.debugEvent)('plugin_request_failed', {
                error: _devtoolsEvents.debugEvent.error(error)
            });
            ...
            res.end(`The DevTools plugin "${plugin.packageName}" failed to handle the request to "${urlInPluginRoot}": ` + `${error?.message ?? error}. This is likely a bug in the plugin's server entry point ` + `(${plugin.serverEntryPoint}); report it to the plugin author.`);
            return true;
        } finally{
            req.url = originalUrl;
        }
