...
const _getNamedParametrizedRoute = require("../../getNamedParametrizedRoute");
const _defineRouter = require("./defineRouter");
function compilePathMatcher(path, suffix) {
    ...
    const { namedParameterizedRoute, routeKeys, wildcardKeys } = (0, _getNamedParametrizedRoute.getNamedParametrizedRoute)(path);
...
function isDynamicPath(path) {
    return path.split('/').some((segment)=>(0, _routing().matchDynamicName)(segment) != null);
}
function hasPathPrefix(prefix, path) {
    return path === prefix || path.startsWith(prefix + '/');
}
/** Normalize a registration path to a URL-shaped pathname (always starts with `/`). */ function normalizePath(path) {
    if (path === '' || path === '/') return '/';
...
function createPages(fn) {
    ...
    const createPage = (page)=>{
        ...
        for (const segment of segments){
            const dynamic = (0, _routing().matchDynamicName)(segment);
            if (!dynamic) continue;
            numSlugs++;
    ...
    return (0, _defineRouter.unstable_defineRouter)(async ()=>{
        await configure();
        ...
        for (const entry of sortedEntries){
            ...
            const WrappedComponent = (props)=>(0, _react().createElement)(entry.component, {
                    ...props,
                    ...mapping
                });
            return {
