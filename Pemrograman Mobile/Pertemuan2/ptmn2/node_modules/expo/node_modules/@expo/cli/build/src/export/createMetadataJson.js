...
function createMetadataJson({ bundles, fileNames, embeddedHashSet, domComponentAssetsMetadata }) {
    ...
    return {
        ...
        fileMetadata: Object.entries(bundles).reduce((metadata, [platform, bundle])=>{
            ...
            const seen = new Set();
            const assets = bundle.assets.filter((asset)=>!embeddedHashSet || !embeddedHashSet.has(asset.hash)).map((asset)=>// Each asset has multiple hashes which we convert and then flatten.
