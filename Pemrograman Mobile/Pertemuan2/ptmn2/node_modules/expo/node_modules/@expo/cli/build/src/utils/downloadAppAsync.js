...
const _events = require("../api/events");
...
async function downloadAsync({ url, outputPath, cacheDirectory, onProgress }) {
    ...
    (0, _events.debugEvent)('download_started', {
        url,
        output: outputPath
    });
    ...
    await pipeline(_stream().Readable.fromWeb(res.body), _fs().default.createWriteStream(outputPath));
    (0, _events.event)('download', {
        url,
        bytes: _fs().default.statSync(outputPath).size,
...
async function downloadAppAsync({ url, outputPath, extract = false, cacheDirectory, onProgress }) {
    if (extract) {
        ...
        const tmpPath = (0, _createTempPath.createTempFilePath)(_path().default.basename(outputPath));
        await downloadAsync({
            url,
            outputPath: tmpPath,
            cacheDirectory,
        ...
        (0, _events.debugEvent)('extract_started', {
            input: tmpPath,
            output: outputPath
        });
