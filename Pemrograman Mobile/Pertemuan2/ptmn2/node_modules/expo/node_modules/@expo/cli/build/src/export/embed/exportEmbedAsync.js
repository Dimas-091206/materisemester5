...
0 && (module.exports = {
    createMetroServerAndBundleRequestAsync: null,
    exportEmbedAssetsAsync: null,
    exportEmbedAsync: null,
    exportEmbedBundleAndAssetsAsync: null,
    exportEmbedInternalAsync: null
});
...
function guessCopiedAppleBundlePath(bundleOutput) {
    ...
    if (!bundleOutput.match(/\/Xcode\/DerivedData\/.*\/Build\/Products\//) && !bundleOutput.match(/\/CoreSimulator\/Devices\/.*\/data\/Containers\/Bundle\/Application\//)) {
        (0, _events.debugEvent)('embed:nonstandard_location', {
            bundleOutput
        });
        return false;
    }
    ...
    (0, _events.debugEvent)('embed:possible_previous_bundle', {
...
async function exportEmbedInternalAsync(projectRoot, options) {
    ...
    if (isApplePlatform(options.platform)) {
        ...
        if (previousPath && _fs().default.existsSync(previousPath)) {
            (0, _events.debugEvent)('embed:removing_previous_bundle', {
                previousPath
            });
...
async function exportEmbedBundleAndAssetsAsync(projectRoot, options) {
    ...
    try {
        ...
        const apiRoutesEnabled = devServer.isReactServerComponentsEnabled || exp.web?.output === 'server';
        ...
        return {
            ...
            bundle: {
                ...
                map: bundles.artifacts.filter((a)=>a.type === 'map')[0]?.source.toString()
            },
