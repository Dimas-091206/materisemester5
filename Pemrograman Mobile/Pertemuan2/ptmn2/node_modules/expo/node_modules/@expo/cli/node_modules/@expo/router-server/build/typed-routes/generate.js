...
Object.defineProperty(exports, "getTypedRoutesDeclarationFile", {
    enumerable: true,
    get: function() {
...
function getTypedRoutesDeclarationFile(ctx, { partialTypedGroups = false, testIgnoreComments = false, unstable_useServerMiddleware = false } = {}) {
    ...
    for (const [dynamicRouteTemplate, paramsNames] of groupedNodes.dynamic){
        const inputParams = paramsNames.map((param)=>{
        ...
        const outputParams = paramsNames.map((param)=>{
            const key = param.startsWith('...') ? param.slice(3) : param;
            const value = param.startsWith('...') ? 'string[]' : 'string';
            return `${contextKeyToProperty(key)}: ${value};`;
        }).join('');
        dynamicRouteStrings.push(contextKeyToType(dynamicRouteTemplate.replaceAll(CATCH_ALL, '${string}').replaceAll(SLUG, '${Router.SingleRoutePart<T>}') + urlParams, partialTypedGroups));
    ...
    const tsExpectError = testIgnoreComments ? '// @ts-ignore-error -- During tests we need to ignore the "duplicate" declaration error, as multiple fixture declare types \n      ' : '';
...
function groupRouteNodes(routeNode, groupedContextKeys = {
    ...
    if (routeNode.generated) {
        ...
        routeKey = (0, _routing().removeSupportedExtensions)(routeNode.contextKey).replace(/\/index$/, '') // Remove any trailing /index
