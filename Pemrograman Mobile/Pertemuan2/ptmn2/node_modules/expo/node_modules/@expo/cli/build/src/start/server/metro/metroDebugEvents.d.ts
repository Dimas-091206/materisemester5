...
declare module '2g' {
    interface EventRegistry {
        'metro:ssr_hmr_registered': {
            url: string;
        };
        'metro:ssr_hmr_unknown_message': {
            type: string;
        };
        'metro:env_reload': Record<string, never>;
        'metro:rsc_route_added': {
            path: string;
        };
        'metro:client_boundaries_evaluated': {
            count: number;
        };
        'metro:client_boundaries_rebundle': {
            count: number;
        };
        'metro:ssr_manifest': {
            boundaryCount: number;
        };
        'metro:bundle_api_route': {
            routerRoot: string;
            path: string;
        };
        'metro:api_route_overlay_failed': {
            error: SerializedError;
        };
        'metro:loader_route_matched': {
            pathname: string;
            file: string;
        };
        'metro:loader_bundle_failed': {
            path: string;
            error: SerializedError;
        };
        'metro:loader_graph_changed': {
            path: string;
        };
...
export declare const debugEvent: import("2g").EventLogger<"metro">;
