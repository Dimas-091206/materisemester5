...
0 && (module.exports = {
    showLoginPromptAsync: null,
    tryGetUserAsync: null
});
...
async function showLoginPromptAsync({ printNewLine = false, otp, ...options } = {}) {
    ...
    const browser = options.browser ?? (!hasCredentials && (0, _interactive.isInteractive)());
    if (printNewLine) {
        _log.log();
    }
    if (sso || browser) {
    ...
    try {
        ...
        if (e instanceof _client.ApiV2Error && e.expoApiV2ErrorCode === 'ONE_TIME_PASSWORD_REQUIRED') {
            await (0, _otp.retryUsernamePasswordAuthWithOTPAsync)(username, password);
        } else {
            throw e;
        }
    }
