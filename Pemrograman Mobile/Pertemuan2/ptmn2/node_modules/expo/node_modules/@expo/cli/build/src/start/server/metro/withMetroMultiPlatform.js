...
0 && (module.exports = {
    getNodejsExtensions: null,
    shouldAliasModule: null,
    withExtendedResolver: null,
    withMetroMultiPlatformAsync: null
});
    ...
    const getPolyfills = (ctx)=>{
        ...
        if (ctx.platform === 'web') {
            try {
                ...
                // want long-term.
                const rnGetPolyfills = require('@react-native/js-polyfills');
                return [
...
function withExtendedResolver(config, { autolinkingModuleResolverInput, isTsconfigPathsEnabled, isExporting, isReactServerComponentsEnabled, getMetroBundler }) {
    ...
    const idFactory = config.serializer?.createModuleIdFactory?.() ?? ((id, context)=>id);
    ...
    const externals = [
        {
            match: (context, moduleName)=>{
                ...
                !(0, _metroOptions.isServerEnvironment)(context.customResolverOptions?.environment)) {
                    return false;
                }
        ...
        // Externals to speed up async split chunks by extern-ing common packages that appear in the root client chunk.
        {
            match: (context, moduleName, platform)=>{
                ...
                (0, _metroOptions.isServerEnvironment)(context.customResolverOptions?.environment) || // Only enable for client boundaries
    ...
    const metroConfigWithCustomResolver = (0, _withMetroResolvers.withMetroResolvers)(config, [
        ...
        function requestDevMockProdReact(context, moduleName, platform) {
            ...
            platform !== 'web' && getReactNativeHostPathPattern(platform)?.test(context.originModulePath) && moduleName.match(/([\\/]ReactFabric|ReactNativeRenderer)-prod/) || // Match react production imports.
        ...
        function requestNodeExternals(context, moduleName, platform) {
            const isServer = context.customResolverOptions?.environment === 'node' || context.customResolverOptions?.environment === 'react-server';
                ...
                if (external.match(context, moduleName, platform)) {
                    if (external.replace === 'empty') {
                        ...
                        const opaqueId = idFactory(realPath, {
                            platform: platform,
                            environment: typeof context.customResolverOptions?.environment === 'string' ? context.customResolverOptions.environment : undefined
                        });
        ...
        function requestStableConfigModules(context, moduleName, platform) {
            ...
            if (moduleName === config.transformer.assetRegistryPath || moduleName === 'react-native/asset-registry' || /^@react-native\/assets-registry\/registry(\.js)?$/.test(moduleName)) {
            ...
            if (moduleName.startsWith('.') && /[\\/]private[\\/]assets[\\/]AssetRegistry(\.js)?$/.test(moduleName) && /[\\/]react-native[\\/](src|Libraries)[\\/]/.test(context.originModulePath)) {
                return getAssetRegistryModule();
            }
    ...
    const metroConfigWithCustomContext = (0, _withMetroResolvers.withMetroMutatedResolverContext)(metroConfigWithCustomResolver, (immutableContext, moduleName, platform)=>{
        ...
        if ((0, _metroOptions.isServerEnvironment)(context.customResolverOptions?.environment)) {
            ...
            const isReactServerComponents = context.customResolverOptions?.environment === 'react-server';
...
function shouldAliasModule(input, alias) {
    return input.platform === alias.platform && input.result?.type === 'sourceFile' && typeof input.result?.filePath === 'string' && normalizeSlashes(input.result.filePath).endsWith(alias.output);
}
