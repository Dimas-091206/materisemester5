...
0 && (module.exports = {
    devicectlAsync: null,
    getConnectedAppleDevicesAsync: null,
    hasDevicectlEverBeenInstalled: null,
    installAndLaunchAppAsync: null,
    launchAppWithDeviceCtl: null,
    launchBinaryOnMacAsync: null
});
...
async function getConnectedAppleDevicesAsync() {
    if (!hasDevicectlEverBeenInstalled()) {
        (0, _events.event)('devicectl_not_found', {});
        return [];
    }
    ...
    if (![
        ...
        3,
        5
    ].includes(devicesJson?.info?.jsonVersion)) {
    ...
    return devicesJson.result.devices.filter((device)=>device?.hardwareProperties?.reality !== 'simulated');
}
function assertDevicesJson(results) {
    (0, _nodeassert().default)(results != null && 'result' in results && Array.isArray(results?.result?.devices), 'Malformed JSON output from devicectl: ' + JSON.stringify(results, null, 2));
}
...
async function installAppWithDeviceCtlInternalAsync(uuid, bundleIdOrAppPath, onProgress, attempt) {
    ...
    return new Promise((resolve, reject)=>{
        ...
        (0, _events.event)('devicectl_install_spawn', {
...
async function installAndLaunchAppAsync(props) {
    (0, _events.event)('devicectl_install_device', {
        props: props
    });
