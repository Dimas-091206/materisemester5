...
async function startInterfaceAsync(devServerManager, options) {
    ...
    const onPressAsync = async (key)=>{
        ...
        if (isWebSocketsEnabled) {
            switch(key){
                case 'm':
                    (0, _events.event)('toggle-dev-menu', {});
                    return actions.toggleDevMenu();
                case 'M':
                    (0, _events.event)('open-more-tools', {});
        ...
        switch(key){
            case 's':
                {
                    (0, _events.event)('toggle-runtime-mode', {});
                    _log.clear();
            ...
            case 'c':
                (0, _events.event)('clear-terminal', {});
                _log.clear();
                await actions.printDevServerInfoAsync(usageOptions);
                return;
            case 'j':
                (0, _events.event)('open-debugger', {});
                return actions.openJsInspectorAsync();
            case 'r':
                (0, _events.event)('reload', {});
                return actions.reloadApp();
            case 'o':
                (0, _events.event)('open-editor', {});
