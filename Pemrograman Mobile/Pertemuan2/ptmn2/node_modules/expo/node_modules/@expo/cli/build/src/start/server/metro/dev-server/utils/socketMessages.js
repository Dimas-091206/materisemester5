"use strict";
Object.defineProperty(exports, "__esModule", {
    value: true
});
0 && (module.exports = {
    parseRawMessage: null,
    serializeMessage: null
});
function _export(target, all) {
    for(var name in all)Object.defineProperty(target, name, {
        enumerable: true,
        get: Object.getOwnPropertyDescriptor(all, name).get
    });
}
_export(exports, {
    get parseRawMessage () {
        return parseRawMessage;
    },
    get serializeMessage () {
        return serializeMessage;
    }
});
const _hmrEvents = require("../../hmrEvents");
/** The current websocket-based communication between Metro, CLI, and client devices */ const PROTOCOL_VERSION = 2;
function parseRawMessage(data, isBinary) {
    if (isBinary) return null;
    try {
        const { version, ...message } = JSON.parse(data.toString()) ?? {};
        if (version === PROTOCOL_VERSION) {
            return message;
        }
        (0, _hmrEvents.event)('protocol_version_mismatch', {
            expected: PROTOCOL_VERSION,
            received: version
        });
    } catch (error) {
        (0, _hmrEvents.event)('message_parse_failed', {
            error: _hmrEvents.event.error(error)
        });
    }
    return null;
}
function serializeMessage(message) {
    return JSON.stringify({
        ...message,
        version: PROTOCOL_VERSION
    });
}

//# sourceMappingURL=socketMessages.js.map
