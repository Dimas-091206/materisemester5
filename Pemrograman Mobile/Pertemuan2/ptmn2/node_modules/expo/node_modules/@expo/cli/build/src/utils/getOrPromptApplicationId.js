...
0 && (module.exports = {
    getOrPromptForBundleIdentifierAsync: null,
    getOrPromptForPackageAsync: null
});
...
async function getRecommendedReverseDomainNameSecondPartAsync(exp) {
    ...
    const cachedUsername = (0, _UserSettings.getSettings)().read().auth?.username;
    if (cachedUsername) {
...
async function promptForBundleIdWithInitialAsync(projectRoot, // oxlint-disable-next-line oxc/only-used-in-recursion
...
async function getRecommendedBundleIdAsync(exp) {
    const possibleIdFromAndroid = exp.android?.package ? (0, _validateApplicationId.getSanitizedBundleIdentifier)(exp.android.package) : undefined;
...
async function getRecommendedPackageNameAsync(exp) {
    const possibleIdFromApple = exp.ios?.bundleIdentifier ? (0, _validateApplicationId.getSanitizedPackage)(exp.ios.bundleIdentifier) : undefined;
    ...
    if (possibleIdFromApple && (0, _validateApplicationId.validatePackage)(possibleIdFromApple)) {
        ...
        if ((0, _validateApplicationId.validatePackage)(possibleId)) {
            ...
            (0, _events.event)('invalid_package_name', {
                name: possibleId,
...
async function promptForPackageWithInitialAsync(projectRoot, // oxlint-disable-next-line oxc/only-used-in-recursion
exp, packageName) {
    if (!packageName) {
