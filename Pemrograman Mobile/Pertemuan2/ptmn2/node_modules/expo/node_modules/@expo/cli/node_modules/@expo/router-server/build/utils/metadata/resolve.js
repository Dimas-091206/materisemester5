...
_export(exports, {
    get resolveAppleWebApp () {
        return resolveAppleWebApp;
    },
    get resolveMetadata () {
        return resolveMetadata;
    },
    get resolveOpenGraph () {
...
function resolveRobots(robots) {
    if (!robots) {
        return {};
    }
    return {
        robots: serializeRobotsValue(robots),
        googleBot: typeof robots === 'object' ? serializeRobotsValue(robots.googleBot) : undefined
    };
}
...
function resolveTwitter(twitter) {
...
function resolveVerification(verification) {
...
function resolveAppleWebApp(appleWebApp) {
...
function resolveBasicMetadata(metadata) {
    return {
...
function resolveAlternates(alternates) {
...
function resolveTwitterApp(app) {
    if (!app) return [];
    const resolved = [];
...
function resolveIconDescriptors(rel, descriptors) {
    const resolved = [];
...
function resolveFormatDetection(formatDetection) {
...
function resolveFacebook(facebook) {
...
function resolvePinterest(pinterest) {
    if (!pinterest || pinterest.richPin == null) return undefined;
    return pinterest.richPin ? 'true' : 'false';
}
function resolveAppLinks(appLinks) {
    if (!appLinks) return undefined;
    const ios = appLinks.ios && (appLinks.ios.url || appLinks.ios.appStoreId || appLinks.ios.appName) ? {
        url: appLinks.ios.url,
        appStoreId: appLinks.ios.appStoreId,
        appName: appLinks.ios.appName
    } : undefined;
    const android = appLinks.android && (appLinks.android.url || appLinks.android.package || appLinks.android.appName) ? {
        url: appLinks.android.url,
        package: appLinks.android.package,
        appName: appLinks.android.appName
    } : undefined;
    const web = appLinks.web && (appLinks.web.url || appLinks.web.shouldFallback != null) ? {
        url: appLinks.web.url,
        shouldFallback: appLinks.web.shouldFallback != null ? appLinks.web.shouldFallback ? 'true' : 'false' : undefined
    } : undefined;
    return {
        ios,
        android,
...
function pushResolvedName(fields, name, content) {
    if (!content) return;
    fields.push({
        name,
        content
...
function pushResolvedProperty(fields, property, content) {
    if (!content) return;
    fields.push({
        property,
        content
