...
0 && (module.exports = {
    _assertXcodeBuildResults: null,
    _extractXcodeBuildErrorLines: null,
    _formatXcodeBuildFailure: null,
    _hasXcodeBuildErrorDetails: null,
    buildAsync: null,
    extractEnvVariableFromBuild: null,
    getAppBinaryPath: null,
    getEscapedPath: null,
    getGenericSimulatorDestination: null,
    getProcessOptions: null,
    getXcodeBuildArgsAsync: null,
    logPrettyItem: null,
    matchEstimatedBinaryPath: null
});
...
_export(exports, {
    ...
    get _formatXcodeBuildFailure () {
        return _formatXcodeBuildFailure;
    },
    get _hasXcodeBuildErrorDetails () {
        return _hasXcodeBuildErrorDetails;
    },
    get buildAsync () {
        return buildAsync;
    },
...
const XCODE_BUILD_NO_OUTPUT_ERROR_MESSAGE = /error: the following command failed with exit code \d+ but produced no further output/;
...
async function buildAsync(props) {
    ...
    if (code !== 0) {
        if (_hasXcodeBuildErrorDetails(formatter.errors)) {
            ...
            throw new _errors.CommandError(_formatXcodeBuildFailure(code, logFilePath));
        }
        _assertXcodeBuildResults(code, results, error, xcodeProject, logFilePath);
    }
    return results;
}
function _formatXcodeBuildFailure(code, logFilePath) {
    return `Failed to build iOS project. "xcodebuild" exited with error code ${code}.\nBuild logs written to ${_chalk().default.underline(logFilePath)}`;
}
function _hasXcodeBuildErrorDetails(errors) {
...
function isXcodeBuildNoOutputErrorLine(line) {
