...
async function runIosAsync(projectRoot, options) {
    ...
    if (props.device) {
        (0, _events.event)('device:selected', {
            platform: 'ios',
    ...
    if (options.rebundle) {
        ...
        if (!props.device) {
            throw new _errors.CommandError('Re-bundling requires a specific device. Cannot use --device generic.');
        }
        let binaryPath;
        if (!options.binary) {
    ...
    if (options.binary) {
        ...
        const done = _events.event.span();
        let buildOutput;
        try {
            buildOutput = await _XcodeBuild.buildAsync({
                ...props,
                eagerBundleOptions
            });
        } catch (error) {
            (0, _events.event)('build:failed', {
                platform: 'ios',
    ...
    if (props.isSimulator) {
        try {
            ...
            (0, _events.debugEvent)('ios:terminate_failed', {
    ...
    const manager = await (0, _startBundler.startBundlerAsync)(projectRoot, {
        ...
        scheme: isCustomBinary ? launchInfo.schemes[0] : (await (0, _scheme.getSchemesForIosAsync)(projectRoot))?.[0]
    });
...
/** Copy the built binary to the specified output directory. */ async function copyBinaryToOutputAsync(binaryPath, outputDir) {
    const absoluteOutputDir = _path().default.resolve(outputDir);
    const appName = _path().default.basename(binaryPath);
    const outputPath = _path().default.join(absoluteOutputDir, appName);
    (0, _events.debugEvent)('ios:binary_copy', {
        path: outputPath
    });
    // Create the output directory if it doesn't exist.
    await _fs().default.promises.mkdir(absoluteOutputDir, {
        recursive: true
    });
    // Copy the .app bundle to the output directory.
    await _fs().default.promises.cp(binaryPath, outputPath, {
        recursive: true
    });
    _log.log((0, _chalk().default)`{dim Copied to} ${outputPath}`);
    return outputPath;
}
