...
0 && (module.exports = {
    event: null,
    instantiateMetroAsync: null,
    isWatchEnabled: null,
    loadMetroConfigAsync: null,
    prewarmTransformPool: null
});
...
_export(exports, {
    ...
    get prewarmTransformPool () {
        return prewarmTransformPool;
    }
});
function _2g() {
...
function _getMaxWorkers() {
    const data = /*#__PURE__*/ _interop_require_default(require("@expo/metro/metro-config/defaults/getMaxWorkers"));
    _getMaxWorkers = function() {
        return data;
    };
    return data;
}
...
const event = (0, _2g().events)('metro');
...
async function loadMetroConfigAsync(projectRoot, options, { exp, isExporting, getMetroBundler }) {
    ...
    const autolinkingModuleResolutionEnabled = exp.experiments?.autolinkingModuleResolution ?? (isWorkspace || targetsOutOfTreePlatform);
    const serverActionsEnabled = exp.experiments?.reactServerFunctions ?? _env.env.EXPO_UNSTABLE_SERVER_FUNCTIONS;
    ...
    const enableNativeTransformWorker = !!exp.experiments?.noxcturnalTransformWorker;
    asWritable(config.transformer).unstable_noxcturnalTransformWorker = enableNativeTransformWorker;
    ...
    if (isExporting) {
        ...
        asWritable(config.transformer).publicPath = `/assets?export_path=${(exp.experiments?.baseUrl ?? '') + '/assets'}`;
    } else {
    ...
    const reduceLogs = (0, _interactive.shouldReduceLogs)();
    ...
    if (!reduceLogs && serverActionsEnabled) {
        _log.Log.warn(`React Server Functions (beta) are enabled. Route rendering mode: ${exp.experiments?.reactServerComponentRoutes ? 'server' : 'client'}`);
    }
...
async function instantiateMetroAsync(metroBundler, options, { isExporting, exp = (0, _config().getConfig)(metroBundler.projectRoot, {
    ...
    const tls = metroConfig.server?.tls;
    ...
    metro._getSortedModules = function(graph) {
        ...
        for (const module1 of modules){
            this._createModuleId(module1.path, ctx);
        }
        // Sort by IDs
...
async function prewarmTransformPool(bundler, maxWorkers) {
    const workers = (0, _getMaxWorkers().default)(maxWorkers);
    if (workers <= 1) {
        return;
    }
    const warmOptions = {
    ...
    await Promise.allSettled(Array.from({
        length: workers
    }, (_, i)=>// Defer each invocation so synchronous transform errors are settled as well. Prewarming is
        // opportunistic and must never prevent the dev server from starting.
        Promise.resolve().then(()=>bundler.transformFile(`/__prewarm__/${i}.js`, warmOptions, defaultSource))));
    done('prewarm', {
        workers
    });
}
...
function pruneCustomTransformOptions(projectRoot, filePath, transformOptions) {
    ...
    if (transformOptions.customTransformOptions?.dom && // The only generated file that needs the dom root is `expo/dom/entry.js`
    ...
    if (transformOptions.customTransformOptions?.asyncRoutes && // The async routes settings are also used in `expo-router/_ctx.ios.js` (and other platform variants) via `process.env.EXPO_ROUTER_IMPORT_MODE`
    !(filePath.match(/\/expo-router\/_ctx/) || filePath.match(/\/expo-router\/(?:build|src)\//))) {
        delete transformOptions.customTransformOptions.asyncRoutes;
    }
    if (transformOptions.customTransformOptions?.clientBoundaries && // The client boundaries are only used in `expo/virtual/rsc.js` for production RSC exports.
