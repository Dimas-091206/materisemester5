...
0 && (module.exports = {
    unstable_defineRouter: null,
    unstable_redirect: null
});
...
_export(exports, {
    get unstable_defineRouter () {
        return unstable_defineRouter;
    },
    get unstable_redirect () {
...
function unstable_defineRouter(getPathConfig, getComponent) {
    ...
    const renderEntries = async (input, { params, buildConfig })=>{
        const pathname = (0, _rsc().parseInputString)(input);
        ...
        const { layouts, page } = (0, _rsc().getComponentIds)(pathname);
        const entries = (await Promise.all([
            ...
            const element = (0, _react().createElement)(resolved.component, isPage ? {
                path: pathname,
                query
            } : {
                path: pathname
            }, (0, _react().createElement)(_rsc().Children));
            return [
                [
                    id,
                    element
    ...
    const getBuildConfig = async (unstable_collectClientModules)=>{
        ...
        for (const { pathname } of pathConfig){
            ...
            const input = (0, _rsc().getInputString)(pathname);
...
globalThis.__EXPO_ROUTER_PREFETCH__ = (path) => {
  ...
  for (const id of path2ids[path] || []) {
    ...
    const getSsrConfig = async (pathname, { searchParams, buildConfig })=>{
        ...
        const { layouts, page } = (0, _rsc().getComponentIds)(pathname);
        const input = (0, _rsc().getInputString)(pathname);
        const html = (0, _react().createElement)(_rsc().ServerRouter, {
            route: {
                path: pathname,
            ...
            ...layouts,
            page
        ].reduceRight((acc, id)=>(0, _react().createElement)(_rsc().Slot, {
                id,
                fallback: acc
            }, acc), null));
        return {
...
function unstable_redirect(pathname, searchParams, skip) {
    if (skip) {
        ...
        for (const id of skip){
            searchParams.append(_rsc().PARAM_KEY_SKIP, id);
        }
    }
    const input = (0, _rsc().getInputString)(pathname);
    (0, _server.rerender)(input, searchParams);
}
