...
0 && (module.exports = {
    AppleDeviceManager: null,
    ensureSimulatorOpenAsync: null
});
...
_export(exports, {
    get AppleDeviceManager () {
        return AppleDeviceManager;
    },
    get ensureSimulatorOpenAsync () {
...
class AppleDeviceManager extends _DeviceManager.DeviceManager {
    ...
    static async resolveAsync({ device, shouldPrompt } = {}) {
        if (shouldPrompt) {
            ...
            device = await (0, _promptAppleDevice.promptAppleDeviceAsync)(devices, device?.osType);
        }
    ...
    async getApplicationIdFromBundle(filePath) {
        ...
        if (_fs().default.existsSync(builtInfoPlistPath)) {
            ...
            (0, _events.event)('apple_bundle_id_resolved', {
                filePath,
                bundleId: CFBundleIdentifier
            });
            return CFBundleIdentifier;
        }
        (0, _events.event)('apple_bundle_id_fallback', {});
    ...
    async isAppInstalledAndIfSoReturnContainerPathForIOSAsync(appId) {
        return await _simctl.getContainerPathAsync(this.device, {
            appId
        }) ?? false;
    }
    ...
    async ensureExpoGoAsync(sdkVersion) {
        if (this.device.osType === 'watchOS') {
            throw new _errors.CommandError('UNSUPPORTED_DEVICE', `Expo Go is not supported on Apple Watch. Please select an iPhone or iPad simulator instead.`);
        }
