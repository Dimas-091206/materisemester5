...
function isNetworkLog(simLog) {
    return simLog.subsystem === 'com.apple.network' || simLog.category === 'connection' || simLog.source?.image === 'CFNetwork';
}
...
function formatMessage(simLog) {
    ...
    const category = _chalk().default.gray(`[${simLog.source?.image ?? simLog.subsystem}]`);
...
function onMessage(simLog) {
    ...
    if (simLog.messageType === 'Error') {
        ...
        if (simLog.source?.file || simLog.eventMessage.includes('Terminating app due to uncaught exception')) {
            hasLogged = true;
