...
0 && (module.exports = {
    DevServerManagerActions: null,
    getDevToolsPluginCliBannerItems: null,
    printDevToolsPluginCliBannersAsync: null
});
...
async function printDevToolsPluginCliBannersAsync(devServerManager) {
    ...
    try {
        ...
        (0, _events.event)('banners-failed', {
...
class DevServerManagerActions {
    ...
    async printDevServerInfoAsync(options) {
        ...
        if (this.options.platforms?.includes('web')) {
            ...
            const webUrl = webDevServer?.getDevServerUrl({
                hostType: 'localhost'
            });
            if (webUrl) {
        ...
        const dependencyCheckLines = (0, _checkDependenciesOnStart.getDependencyCheckMessage)(options.dependencyCheckRef?.result);
    ...
    async openMoreToolsAsync() {
        ...
        try {
            ...
            if (menuItem?.action) {
                (0, _events.event)('run-menu-action', {
                    action: menuItem.value
                });
                menuItem.action();
            } else if (menuItem?.value) {
                (0, _events.event)('send-dev-command', {
                    commandName: menuItem.value
                });
