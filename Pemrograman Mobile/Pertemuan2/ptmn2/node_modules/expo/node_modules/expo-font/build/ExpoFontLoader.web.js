...
function getFontFaceRulesMatchingResource(fontFamilyName, options) {
    ...
    return rules.filter(({ rule })=>{
        return normalizeFontFamilyName(rule.style.fontFamily) === fontFamilyName && (options && options.display ? options.display === rule.style.fontDisplay : true);
    });
}
...
const toExport = isServer ? ExpoFontLoader : // Normally that'd be a class but that doesn't work on server, so we use a function instead.
...
export function _createWebFontTemplate(fontFamily, resource) {
    const display = typeof resource.display === 'string' && CSS_IDENT_RE.test(resource.display) ? resource.display : FontDisplay.AUTO;
