...
Object.defineProperty(exports, "environmentRestrictedReactAPIsPlugin", {
    enumerable: true,
    get: function() {
        return environmentRestrictedReactAPIsPlugin;
    }
});
        ...
        visitor: {
                ...
                if (forbiddenList) {
                    path.node.specifiers.forEach((specifier)=>{
                        if (t.isImportSpecifier(specifier)) {
                            const importName = t.isStringLiteral(specifier.imported) ? specifier.imported.value : specifier.imported.name;
                            ...
                            if (isForbidden) {
                                if ([
                                    ...
                                    if (!forbiddenImports.has(sourceValue)) forbiddenImports.set(sourceValue, new Set());
                                    forbiddenImports.get(sourceValue).add(importName);
                                    path.scope.setData('forbiddenImports', forbiddenImports);
                                }
                            }
                        } else {
                            const importName = t.isStringLiteral(specifier.local) ? specifier.local : specifier.local.name;
                            // Save namespace import for later checks in MemberExpression
                            path.scope.setData('importedNamespace', {
            ...
            MemberExpression (path) {
                ...
                Object.keys(importedNamespaces).forEach((namespace)=>{
                    ...
                    if (objectName === namespace && forbiddenList && t.isIdentifier(path.node.property) && forbiddenList.includes(path.node.property.name)) {
