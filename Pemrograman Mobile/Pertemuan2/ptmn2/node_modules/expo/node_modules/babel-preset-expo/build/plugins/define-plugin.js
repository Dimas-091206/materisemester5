...
function definePlugin({ types: t }) {
   ...
   */ function replaceAndEvaluateNode(nodePath, replacement) {
    ...
    /** Get the root identifier name from a member expression (e.g., "process" from "process.env.NODE_ENV") */ function getMemberExpressionRoot(node) {
        let current = node;
    ...
    return {
        ...
        visitor: {
            ...
            ReferencedIdentifier (nodePath, state) {
                ...
                if (container && !Array.isArray(container) && 'type' in container && (container.type === 'ImportDefaultSpecifier' || container.type === 'ImportSpecifier')) {
                    return;
                }
                // Do not transform Object keys / properties unless they are computed like {[key]: value}
                if ((nodePath.key === 'key' || nodePath.key === 'property') && nodePath.parent && 'computed' in nodePath.parent && nodePath.parent.computed === false) {
                    return;
                }
