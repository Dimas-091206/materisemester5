...
Object.defineProperty(exports, "expoUseDomDirectivePlugin", {
    enumerable: true,
    get: function() {
        return expoUseDomDirectivePlugin;
    }
});
...
function expoUseDomDirectivePlugin(api) {
    ...
    const projectRoot = api.caller(_common.getPossibleProjectRoot);
    return {
        ...
        visitor: {
            Program (path, state) {
                ...
                path.traverse({
                    ExportNamedDeclaration (path) {
                        ...
                        if (t.isTypeAlias(declaration) || t.isInterfaceDeclaration(declaration) || t.isTSTypeAliasDeclaration(declaration) || t.isTSInterfaceDeclaration(declaration)) {
                            // Allows type exports
                            return;
                        }
                ...
                const fileBasename = (0, _nodepath().basename)(filePath);
                if (projectRoot && // Detecting if the file is in the router root would be extensive as it would cause a more complex
                ...
                filePath.includes(projectRoot) && !filePath.match(/node_modules/)) {
          ...
          ${isProduction ? `const filePath = "${_nodecrypto().default.createHash('md5').update(outputKey).digest('hex')}.html";` : `const filePath = "${fileBasename}?file=" + ${JSON.stringify(outputKey)};`}
        ...
        `;
                ...
                results.forEach((nodePath)=>{
                    if (t.isVariableDeclaration(nodePath.node) && nodePath.node.declarations[0]?.id != null && 'name' in nodePath.node.declarations[0].id && nodePath.node.declarations[0].id.name === '_Expo_DOMProxyComponent') {
...
//# sourceMappingURL=use-dom-directive-plugin.js.map
