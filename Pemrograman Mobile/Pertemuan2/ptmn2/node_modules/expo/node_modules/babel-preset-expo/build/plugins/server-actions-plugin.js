...
Object.defineProperty(exports, "reactServerActionsPlugin", {
    enumerable: true,
    get: function() {
        return reactServerActionsPlugin;
    }
});
function _nodepath() {
...
function reactServerActionsPlugin(api) {
    ...
    const possibleProjectRoot = api.caller(_common.getPossibleProjectRoot);
    let addReactImport;
    let wrapBoundArgs;
    ...
    const extractInlineActionToTopLevel = (path, _state, { body, freeVariables })=>{
        ...
        const extractedFunctionExpr = wrapInRegister(isArrowFn ? t.arrowFunctionExpression(extractedFunctionParams, t.blockStatement(extractedFunctionBody), true) : t.functionExpression(path.node.id, extractedFunctionParams, t.blockStatement(extractedFunctionBody), false, true), extractedIdentifier.name);
        ...
        if (topLevelDecl) {
            ...
            const targetPath = topLevelDecl.isVariableDeclarator() ? topLevelDecl.parentPath : topLevelDecl;
            ...
            const lastImportPath = (Array.isArray(programBody) ? programBody : [
                programBody
            ]).findLast((statement)=>{
    ...
    return {
        ...
        pre (file) {
            ...
            const addNamedImportOnce = (0, _common.createAddNamedImportOnce)(t);
            addReactImport = ()=>{
            ...
            getActionModuleId = once(()=>{
                ...
                return './' + (0, _common.toPosixPath)((0, _nodepath().relative)(projectRoot, file.opts.filename));
            });
        ...
        visitor: {
            ...
            FunctionDeclaration (path, state) {
                ...
                if (tlb) {
                    ...
                    const bindingKind = 'var';
                    const [inserted] = path.replaceWith(t.variableDeclaration(bindingKind, [
            ...
            ExportDefaultDeclaration (path, state) {
                ...
                if (path.node.declaration) {
                    if (t.isFunctionDeclaration(path.node.declaration)) {
                        ...
                        path.insertAfter(t.exportNamedDeclaration(null, [
                            exportedSpecifier
                        ]));
                    } else {
                        ...
                        if (t.isAssignmentExpression(path.node.declaration) && t.isArrowFunctionExpression(path.node.declaration.right)) {
                            ...
                            path.replaceWith(t.variableDeclaration('var', [
                                t.variableDeclarator(id, right)
                            ]));
                            // Insert `(() => _registerServerReference(foo, "file:///unknown", "default"))();`
                            path.insertAfter(t.exportNamedDeclaration(null, [
                                exportedSpecifier
                            ]));
                        } else if (t.isArrowFunctionExpression(path.node.declaration) && path.node.declaration) {
                        ...
                        } else if (// Match `export default foo;`
                        t.isIdentifier(path.node.declaration)) {
                            ...
                            const isServerActionType = t.isFunctionDeclaration(binding?.path.node ?? path.node.declaration) || t.isArrowFunctionExpression(binding?.path.node ?? path.node.declaration) || // `const foo = async () => {}`
                            t.isVariableDeclarator(binding?.path.node) && t.isArrowFunctionExpression(binding?.path.node.init);
            ...
            ExportNamedDeclaration (path, state) {
                ...
                const identifiers = (()=>{
                    ...
                    if (innerPath.isVariableDeclaration()) {
                        ...
                        return [
                            innerPath.get('id').node
                        ];
                    } else if (// TypeScript type exports
                    innerPath.isTypeAlias() || innerPath.isTSDeclareFunction() || innerPath.isTSInterfaceDeclaration() || innerPath.isTSTypeAliasDeclaration()) {
                        return [];
                    } else {
        ...
        post (file) {
            ...
            const outputKey = _nodeurl().default.pathToFileURL(filePath).href;
...
const getFreeVariables = (path)=>{
    ...
    path.traverse({
        Identifier (innerPath) {
            ...
            if (// function args or a var at the top-level of its body
            binding.scope === path.scope || // decls from blocks within the function
            isChildScope({
                parent: path.scope,
                child: binding.scope,
                root: programScope
...
function findImmediatelyEnclosingDeclaration(path) {
    ...
    while(!currentPath.isProgram()){
        if (// const foo = async () => { ... }
        //       ^^^^^^^^^^^^^^^^^^^^^^^^^
        currentPath.isVariableDeclarator() || // async function foo() { ... }
        // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^
