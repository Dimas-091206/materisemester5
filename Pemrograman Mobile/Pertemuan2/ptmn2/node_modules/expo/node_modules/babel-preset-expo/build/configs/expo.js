...
const _clientmoduleproxyplugin = require("../plugins/client-module-proxy-plugin");
const _environmentrestrictedimports = require("../plugins/environment-restricted-imports");
const _expoinlinemanifestplugin = require("../plugins/expo-inline-manifest-plugin");
const _exporouterplugin = require("../plugins/expo-router-plugin");
const _importmetatransformplugin = require("../plugins/import-meta-transform-plugin");
const _inlineenvvars = require("../plugins/inline-env-vars");
const _lazydecoratorsplugin = require("../plugins/lazy-decorators-plugin");
const _restrictedreactapiplugin = require("../plugins/restricted-react-api-plugin");
const _serveractionsplugin = require("../plugins/server-actions-plugin");
const _serverdataloadersplugin = require("../plugins/server-data-loaders-plugin");
const _servermetadataplugin = require("../plugins/server-metadata-plugin");
const _usedomdirectiveplugin = require("../plugins/use-dom-directive-plugin");
const _widgetsplugin = require("../plugins/widgets-plugin");
const _resolveModule = require("../utils/resolveModule");
...
module.exports = function(api, options) {
    ...
    if (options.inlineEnvironmentVariables) {
        plugins.push(_inlineenvvars.expoInlineEnvVars);
    }
    ...
    if (options.bundler !== 'webpack') {
        plugins.push(_expoinlinemanifestplugin.expoInlineManifestPlugin);
    }
    if ((0, _resolveModule.hasModule)(api, 'expo-router/package.json')) {
        plugins.push(_exporouterplugin.expoRouterBabelPlugin);
        plugins.push(_servermetadataplugin.serverMetadataPlugin);
        ...
        if (!options.isReactServer) {
            plugins.push(_serverdataloadersplugin.serverDataLoadersPlugin);
        }
    }
    plugins.push(_clientmoduleproxyplugin.reactClientReferencesPlugin);
    ...
    if (options.isReactServer) {
        plugins.push(_serveractionsplugin.reactServerActionsPlugin);
        plugins.push(_restrictedreactapiplugin.environmentRestrictedReactAPIsPlugin);
    } else {
        // DOM components must run after "use client" and only in client environments.
        plugins.push(_usedomdirectiveplugin.expoUseDomDirectivePlugin);
    }
    ...
    plugins.push(_environmentrestrictedimports.environmentRestrictedImportsPlugin);
    ...
    if ((0, _resolveModule.hasModule)(api, 'expo-widgets/package.json')) {
        plugins.push(_widgetsplugin.widgetsPlugin);
    }
    ...
    if (options.disableImportExportTransform) {
        plugins.push([
            require('../plugins/detect-dynamic-exports').detectDynamicExports
        ]);
    }
    const polyfillImportMeta = options.transformImportMeta !== false;
    plugins.push((0, _importmetatransformplugin.expoImportMetaTransformPluginFactory)(polyfillImportMeta === true));
    // TODO(@kitten): Remove / deprecated
    plugins.push(...(0, _lazydecoratorsplugin.lazyDecoratorsPlugins)(options.lazyDecorators));
    ...
    if (options.worklets !== false && options.reanimated !== false) {
        const workletsPluginPath = (0, _resolveModule.resolveModule)(api, 'react-native-worklets/plugin');
        if (workletsPluginPath) {
            plugins.push([
                require(workletsPluginPath)
            ]);
        }
    } else if (options.reanimated !== false) {
        const reanimatedPluginPath = (0, _resolveModule.resolveModule)(api, 'react-native-reanimated/plugin');
        if (reanimatedPluginPath) {
            plugins.push([
                require(reanimatedPluginPath)
            ]);
        }
    }
    if (options.expoUi !== false) {
        const expoUiPluginPath = (0, _resolveModule.resolveModule)(api, '@expo/ui/babel-plugin');
        if (expoUiPluginPath) {
            plugins.push([
                require(expoUiPluginPath)
            ]);
        }
    }
    return {
        presets: [
            getReactPreset(options)
        ],
...
function getReactCompilerPlugin(options) {
    if (!options.isReactCompilerEnabled || // Don't run compiler on node modules, it can only safely be run on the user's code.
    options.isNodeModule || // Only run for client code. It's unclear if compiler has any benefits for React Server Components.
    // NOTE: We might want to allow running it to prevent hydration errors.
    options.isServerEnv || // Give users the ability to opt-out of the feature, per-platform.
    ...
    return [
        require('babel-plugin-react-compiler'),
        {
            ...
            customOptOutDirectives: [
                ...reactCompilerOptOutDirectives
            ]
        }
    ];
}
function getReactRefreshPlugin(options) {
    if (!options.enableReactFastRefresh && (!options.isFastRefreshEnabled || options.enableReactFastRefresh === false)) {
        return null;
    } else {
        return [
...
function getCodegenPlugin() {
    return [
        require('@react-native/babel-plugin-codegen'),
        {},
    ...
    return [
        {
            ...
            plugins: [
                [
                    require('../plugins/warn-on-deep-rn-imports')
                ]
            ]
        }
    ];
