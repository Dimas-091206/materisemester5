...
Object.defineProperty(exports, "environmentRestrictedImportsPlugin", {
    enumerable: true,
    get: function() {
        return environmentRestrictedImportsPlugin;
    }
});
...
const FORBIDDEN_REACT_SERVER_IMPORTS = [
    'client-only'
];
function environmentRestrictedImportsPlugin(api) {
    const { types: t } = api;
    const isReactServer = api.caller(_common.getIsReactServer);
    const forbiddenPackages = isReactServer ? FORBIDDEN_REACT_SERVER_IMPORTS : FORBIDDEN_CLIENT_IMPORTS;
    ...
    return {
        ...
        visitor: {
            ...
            CallExpression (path) {
                if (('name' in path.node.callee && path.node.callee.name === 'require' || t.isMemberExpression(path.node.callee) && 'name' in path.node.callee.property && [
                    'resolveWeak',
                    'importAll',
                    'importDefault'
                ].includes(path.node.callee.property.name)) && path.node.arguments.length > 0 && t.isStringLiteral(path.node.arguments[0])) {
                    checkSource(path.node.arguments[0].value, path);
                } else if (path.node.callee.type === 'Import' && path.node.arguments.length > 0 && t.isStringLiteral(path.node.arguments[0])) {
                    checkSource(path.node.arguments[0].value, path);
                }
            }
        }
