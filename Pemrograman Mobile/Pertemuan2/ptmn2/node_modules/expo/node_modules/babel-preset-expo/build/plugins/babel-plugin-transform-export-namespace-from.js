...
Object.defineProperty(exports, // Original: https://github.com/babel/babel/blob/e5c8dc7330cb2f66c37637677609df90b31ff0de/packages/babel-plugin-transform-export-namespace-from/src/index.ts
...
const _default = ({ types: t })=>({
        name: 'transform-export-namespace-from',
        manipulateOptions: process.env.BABEL_8_BREAKING ? undefined : (_, parser)=>parser.plugins.push('exportNamespaceFrom'),
        visitor: {
            ExportNamedDeclaration (path) {
                ...
                const nodes = [];
                if (index === 1) {
                    nodes.push(t.exportNamedDeclaration(null, [
                        specifiers.shift()
                    ], node.source));
                }
                const specifier = specifiers.shift();
                const { exported } = specifier;
                const uid = scope.generateUidIdentifier(// @ts-expect-error Identifier ?? StringLiteral
                exported.name ?? exported.value);
                nodes.push(withLocation(t.importDeclaration([
                    t.importNamespaceSpecifier(uid)
                ], // @ts-expect-error
                t.cloneNode(node.source)), node.loc), withLocation(t.exportNamedDeclaration(null, [
