...
Object.defineProperty(exports, "default", {
    enumerable: true,
    get: function() {
        return minifyPlatformSelectPlugin;
    }
});
function minifyPlatformSelectPlugin({ types: t }) {
    function isPlatformSelect(path) {
        return t.isMemberExpression(path.node.callee) && t.isIdentifier(path.node.callee.object) && t.isIdentifier(path.node.callee.property) && path.node.callee.object.name === 'Platform' && path.node.callee.property.name === 'select' && t.isObjectExpression(path.node.arguments[0]);
    }
    function findProperty(objectExpression, key, fallback) {
        ...
        for (const p of objectExpression.properties){
            ...
            if (t.isIdentifier(p.key) && p.key.name === key || t.isStringLiteral(p.key) && p.key.value === key) {
