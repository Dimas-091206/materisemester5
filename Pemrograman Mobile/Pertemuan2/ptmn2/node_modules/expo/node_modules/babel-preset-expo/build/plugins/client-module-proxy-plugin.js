...
Object.defineProperty(exports, "reactClientReferencesPlugin", {
    enumerable: true,
    get: function() {
        return reactClientReferencesPlugin;
    }
});
function _nodepath() {
...
function reactClientReferencesPlugin(api) {
    ...
    return {
        ...
        visitor: {
            Program (path, state) {
                const isUseClient = path.node.directives.some((directive)=>directive.value.value === 'use client' || // Convert DOM Components to client proxies in React Server environments.
                ...
                const outputKey = './' + (0, _common.toPosixPath)((0, _nodepath().relative)(projectRoot, filePath));
                ...
                if (isUseServer) {
                    ...
                    path.traverse({
                        AssignmentExpression (path) {
                            if (types.isMemberExpression(path.node.left) && 'name' in path.node.left.object && (path.node.left.object.name === 'module' || path.node.left.object.name === 'exports')) {
                        ...
                        CallExpression (path) {
                            if (types.isMemberExpression(path.node.callee) && 'name' in path.node.callee.property && 'name' in path.node.callee.object && path.node.callee.property.name === 'assign' && (path.node.callee.object.name === 'Object' || path.node.callee.object.name === 'exports')) {
                                throw path.buildCodeFrameError('Assignment to `module.exports` or `exports` is not allowed in a "use server" file. Only async functions can be exported.');
                            }
                        }
                    });
                    ...
                    state.file.metadata.proxyExports = [
                        ...proxyExports
                    ];
                    // Save the server action reference in the metadata.
                    state.file.metadata.reactServerReference = _nodeurl().default.pathToFileURL(filePath).href;
                } else if (isUseClient) {
                    if (!isReactServer) {
                    ...
                    if (filePath.endsWith('@react-native/js-polyfills/console.js') || filePath.endsWith('@react-native\\js-polyfills\\console.js')) {
                        // Clear the body
                        path.node.body = [];
                        path.node.directives = [];
                        return;
                ...
                throw new Error(${JSON.stringify(`Attempted to call the default export of ${filePath} from the server but it's on the client. ` + `It's not possible to invoke a client function from the server, it can ` + `only be rendered as a Component or passed to props of a Client Component.`)});
                ...
                throw new Error(${JSON.stringify(`Attempted to call ${exportName}() of ${filePath} from the server but ${exportName} is on the client. ` + `It's not possible to invoke a client function from the server, it can ` + `only be rendered as a Component or passed to props of a Client Component.`)});
                }, ${JSON.stringify(outputKey)}, ${JSON.stringify(exportName)});`);
                        }
                    };
                    ...
                    state.file.metadata.reactClientReference = _nodeurl().default.pathToFileURL(filePath).href;
                }
            }
        }
    };
