...
_export(exports, {
    get RootPathUtils () {
        return RootPathUtils;
    },
    get getAncestorOfRootIdx () {
        return getAncestorOfRootIdx;
    },
    get pathsToPattern () {
        return pathsToPattern;
    }
});
function _invariant() {
 ...
 */ const UP_FRAGMENT_SEP = '..' + _path().default.sep;
const SEP_UP_FRAGMENT = _path().default.sep + '..';
const UP_FRAGMENT_SEP_LENGTH = UP_FRAGMENT_SEP.length;
const CURRENT_FRAGMENT = '.' + _path().default.sep;
const IS_WIN32 = _path().default.sep === '\\';
...
class RootPathUtils {
    ...
    constructor(rootDir){
        ...
        const rootDirnames = [];
        for(let next = rootDir, previous = null; previous !== next; previous = next, next = _path().default.dirname(next)){
            rootDirnames.push(next);
        }
        this.#rootDirnames = rootDirnames;
        this.#rootDepth = rootDirnames.length - 1;
        const rootParts = rootDir.split(_path().default.sep);
    ...
    absoluteToNormal(absolutePath) {
        ...
        for(let nextPart = this.#rootParts[0], nextLength = nextPart.length; nextPart != null && // Check that absolutePath is equal to nextPart + '/' or ends with
        // nextPart, starting from endOfMatchingPrefix.
        absolutePath.startsWith(nextPart, endOfMatchingPrefix) && (absolutePath.length === endOfMatchingPrefix + nextLength || absolutePath[endOfMatchingPrefix + nextLength] === _path().default.sep);){
    ...
    #slowAbsoluteToNormal(absolutePath) {
        const endsWithSep = absolutePath.endsWith(_path().default.sep);
        const result = _path().default.relative(this.#rootDir, absolutePath);
        return endsWithSep && !result.endsWith(_path().default.sep) ? result + _path().default.sep : result;
    }
    ...
    normalToAbsolute(normalPath) {
        ...
        let i = 0;
        let pos = 0;
        while(normalPath.startsWith(UP_FRAGMENT_SEP, pos) || normalPath.endsWith('..') && normalPath.length === 2 + pos){
    ...
    relativeToNormal(relativePath) {
        return this.#tryCollapseIndirectionsInSuffix(relativePath, 0, 0)?.collapsedPath ?? _path().default.relative(this.#rootDir, _path().default.join(this.#rootDir, relativePath));
    }
    resolveSymlinkToNormal(symlinkNormalPath, readlinkResult) {
        let target = (0, _normalizePathSeparatorsToSystem.default)(readlinkResult);
        ...
        if (!isFsRoot && target[len - 1] === _path().default.sep) {
            target = target.slice(0, -1);
        }
        if (_path().default.isAbsolute(target)) {
        ...
        const sepIdx = symlinkNormalPath.lastIndexOf(_path().default.sep);
        const rootRelativeTarget = sepIdx === -1 ? target : symlinkNormalPath.slice(0, sepIdx) + _path().default.sep + target;
    ...
    joinNormalToRelative(normalPath, relativePath) {
        ...
        const left = normalPath.endsWith(_path().default.sep) ? normalPath : normalPath + _path().default.sep;
    ...
    relative(from, to) {
        return _path().default.relative(from, to);
    }
    // Internal: Tries to collapse sequences like `../root/foo` for root
    // `/project/root` down to the normal 'foo'.
    #tryCollapseIndirectionsInSuffix(fullPath, startOfRelativePart, implicitUpIndirections// 0=root-relative, 1=dirname(root)-relative...
    ) {
        ...
        for(let pos = startOfRelativePart;; pos += UP_FRAGMENT_SEP_LENGTH){
            ...
            if (nextIndirection === -1) {
                ...
                while(totalUpIndirections > 0){
                    ...
                    if (fullPath.startsWith(segmentToMaybeCollapse, pos) && // The following character should be either a separator or end of
                    // string
                    (fullPath.length === segmentToMaybeCollapse.length + pos || fullPath[segmentToMaybeCollapse.length + pos] === _path().default.sep)) {
                ...
                if (pos >= fullPath.length) {
                    return {
                        collapsedPath: totalUpIndirections > 0 ? UP_FRAGMENT_SEP.repeat(totalUpIndirections - 1) + '..' + fullPath.slice(pos - 1) : '',
                        collapsedSegments
                    };
                }
...
function getAncestorOfRootIdx(normalPath) {
    ...
    if (normalPath.length === pos + 2 && normalPath.charCodeAt(pos) === 46 && normalPath.charCodeAt(pos + 1) === 46) {
...
function pathsToPattern(paths, pathUtils) {
    ...
    const pathsPatterns = paths.map((input)=>{
        ...
        if (!pattern.endsWith(_path().default.sep)) {
            pattern += _path().default.sep;
        }
...
//# sourceMappingURL=RootPathUtils.js.map
