...
function _fbwatchman() {
    const data = /*#__PURE__*/ _interop_require_default(require("fb-watchman"));
...
const _isWatcherExcluded = /*#__PURE__*/ _interop_require_default(require("../../lib/isWatcherExcluded"));
...
const _planQuery = require("./planQuery");
...
async function watchmanCrawl({ abortSignal, computeSha1, extensions, ignore, includeSymlinks, onStatus, perfLogger, previousState, rootDir, roots }) {
        ...
        try {
            const response = await new Promise((resolve, reject)=>{
                ...
                return client.command([
                    command,
                    ...args
                ], (error, result)=>error ? reject(makeWatchmanError(error)) : resolve(result));
            });
    ...
    async function queryWatchmanForDirs(rootProjectDirMappings) {
        ...
        await Promise.all(Array.from(rootProjectDirMappings).map(async ([posixSeparatedRoot, { directoryFilters, watcher }], index)=>{
            ...
            const since = previousState.clocks.get((0, _normalizePathSeparatorsToPosix.default)(pathUtils.absoluteToNormal((0, _normalizePathSeparatorsToSystem.default)(posixSeparatedRoot))));
            perfLogger?.annotate({
                bool: {
    ...
    for (const [watchRoot, response] of results){
        ...
        newClocks.set((0, _normalizePathSeparatorsToPosix.default)(relativeFsRoot), // Ensure we persist only the local clock.
        typeof response.clock === 'string' ? response.clock : response.clock.clock);
        for (const fileData of response.files){
            const filePath = fsRoot + _path().sep + (0, _normalizePathSeparatorsToSystem.default)(fileData.name);
            ...
            // response is unknown, but there's nothing we need to do in that case.
            } else if (!(0, _isWatcherExcluded.default)(fileData.name) && !ignore(filePath)) {
                const { mtime_ms, size } = fileData;
                (0, _invariant().default)(mtime_ms != null && size != null, 'missing file data in watchman response');
                ...
                if (typeof symlinkInfo === 'string') {
                    symlinkInfo = (0, _normalizePathSeparatorsToPosix.default)(pathUtils.resolveSymlinkToNormal(relativeFilePath, symlinkInfo));
                }
                const nextData = [
                    mtime,
                    size,
