...
const BABEL_CODE_FRAME_ERROR_FORMAT = // Adjusted from original to not capture import stack a part of the code frame
/^(?:TransformError )?(?:.*):? (?:.*?)([/|\\].*): ([\s\S]+?)\n((?:[ >]*\d+[\s|]+[^\n]*\n?)+|\u{001b}\[[0-9;]*m(?:.*\n?)+?(?=\n\n|\n[^\u{001b}\s]|$))/mu;
...
export function parseMetroError(message) {
...
export function parseBabelTransformError(message) {
...
export function parseBabelCodeFrameError(message) {
