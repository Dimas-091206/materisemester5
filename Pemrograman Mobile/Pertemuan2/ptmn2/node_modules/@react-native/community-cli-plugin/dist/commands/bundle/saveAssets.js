...
async function saveAssets(assets, platform, assetsDest, assetCatalogDest) {
  ...
  const addAssetToCopy = (asset) => {
    ...
    asset.scales.forEach((scale, idx) => {
      ...
      const dest = _nodePath.default.join(
        assetsDest,
  ...
  if (platform === "ios" && assetCatalogDest != null) {
    const catalogDir = _nodePath.default.join(
      assetCatalogDest,
      "RNAssets.xcassets",
    );
    if (!_nodeFs.default.existsSync(catalogDir)) {
      console.error(
        `${(0, _nodeUtil.styleText)("red", "error")}: Could not find asset catalog 'RNAssets.xcassets' in ${assetCatalogDest}. Make sure to create it if it does not exist.`,
      );
      return;
    }
    ...
    for (const asset of assets) {
      if ((0, _assetCatalogIOS.isCatalogAsset)(asset)) {
        (0, _assetCatalogIOS.writeImageSet)(
          (0, _assetCatalogIOS.getImageSet)(catalogDir, asset),
        );
      } else {
...
function copy(src, dest, callback) {
  const destDir = _nodePath.default.dirname(dest);
  _nodeFs.default.mkdir(
    destDir,
    {
      recursive: true,
    },
    (err) => {
      ...
      _nodeFs.default
        ...
        .pipe(_nodeFs.default.createWriteStream(dest))
        .on("finish", callback);
    },
  );
}
